...
| 71 | **Core Web Vitals Optimisation** — Mobile 56→85+, Desktop 52→90+. Fixes: (1) render-blocking Google Fonts @import → next/font/google self-hosted; (2) remove `unoptimized:true` + add remotePatterns → WebP/AVIF auto-conversion; (3) hero fetchPriority="high" + next/image priority; (4) favicon 301KB PNG → small optimised icons; (5) preconnect hints; (6) dynamic imports for below-fold client components | **pending — HIGH PRIORITY** |
