...
| **Step 78 — Trip Reports + Trail Conditions (shared backend + web surfaces)** (2026-06-24) | **done** — Alembic `20260624_0049_trip_reports` (`trip_reports` + `trek_media` tables, 3 indexes each); `TripReport`/`TrekMedia` ORM in `modules/reports/models.py` + `db/base.py` registration; `ReportIn`/`ReportOut`/`MediaOut`/`ConditionSummary`/`ReportPageOut`/`MediaUploadOut`/`ModerationIn` Pydantic schemas; service: `upload_media` (JPEG/PNG/WebP, max 5MB, Pillow resize→1920px, boto3 DO Spaces, dev fallback), `create_report`, `get_reports_for_trek` (paginated + condition_summary), `moderate_report`, `delete_report`, `get_moderation_queue`, `get_condition_summary`; 3 router groups: `public_router` (`GET /public/treks/{slug}/reports`), `auth_router` (`POST /reports`, `POST /reports/media/upload`, `DELETE /reports/{id}`), `admin_router` (`GET /admin/reports`, `PATCH /admin/reports/{id}/moderate`); `Pillow>=10.0.0` added to pyproject.toml; 8 tests TC-B-M17-01–TC-B-M17-08, 727/729 pass (2 pre-existing). Web: `lib/reports.ts` (TypeScript interfaces + API functions); `components/trek/ConditionSummaryBanner.tsx`; `components/trek/PhotoGallery.tsx` (full-screen overlay, keyboard nav); `components/trek/TripReportCard.tsx`; `components/trek/AddReportForm.tsx` (controlled form, live char count, immediate photo upload); `components/trek/TrekReportsSection.tsx` (load-more, auth-gate); `app/(public)/trek/[slug]/page.tsx` wired with `<TrekReportsSection>`; `app/(admin)/admin/reports/page.tsx` (moderation queue with pending/approved/rejected tabs, approve/reject with reason); admin layout: "Community" nav group added. `next build` ✓; 7 new URLs added to URL_MAP |
| **TrekSage hotfix 4 — post-77 UX fixes** (2026-06-18) | **done** — (1) Extra footer space: new `treksage/layout.tsx` (Header-only, no footer/TrekSageWidget); (2) Image errors: `onError` fallback on all Image components + added `trekyatra.co.in` / `**.trekyatra.co.in` to next.config.mjs remotePatterns; (3) Homepage pills: `?q=` URL param + TreksageChat `initialQuery` prop auto-sends on mount; (4) Canvas cards lost on back-nav: persist canvasCards to localStorage per session key; (5) Trek detail panel: fetches full TrekProfile on View Details (content_sections, faqs, permit, themes, suitability badges, month guide, accordion sections); (6) Mobile: richer trek cards (stats row, budget, View Trek → /trek/[slug], Compare buttons, compare bar); multi-stage thinking animation; session canvas persistence; next build ✓; tsc ✓ |
| **Step M08 — Trek Comparison (full attribute table + saved comparisons)** (2026-06-18) | **done** — Winner badges per attribute row (budget/permit/beginner-friendly/solo-friendly/family-friendly/crowd_level); Save comparison button (auth-gated, POST /api/v1/account/comparisons); `saved.tsx` converted to stack (`saved/_layout.tsx` + `saved/index.tsx` + `saved/comparisons.tsx`); `useComparisons` hook (list/save/delete); `accountApi` + `apiDelete` added to `mobileApi.ts`; tsc ✓ zero errors |
| **Step M11 — Operators Marketplace** (2026-06-22) | **done** — Operators listing (`GET /api/v1/operators?region=`, search + 6 region chips, `OperatorCard` with GlassSurface + initials avatar + rating + specialities); operator detail screen (hero, about, trek portfolio chips, trek_types, reviews via `GET /api/v1/operators/{slug}/reviews`); `OperatorInquirySheet` modal (`POST /api/v1/inquiries`, name/email/phone/trek_interest/message, success state); call CTA via `Linking.openURL(tel:)`; `useOperators` + `useOperatorDetail` + `useOperatorReviews` + `useSubmitInquiry` hooks; `operatorsApi` namespace + `OperatorSpecialization`/`OperatorReview`/`InquiryPayload`/`InquiryResponse` types added to `mobileApi.ts`; `Operator.region: string[]` bug fixed; browse `_layout.tsx` extended with 2 new Stack.Screen entries; tsc ✓ zero errors |
| **Step M12 — Digital Products** (2026-06-22) | **done** — Product catalog (`GET /api/v1/products`); product detail with purchase CTA; Razorpay native payment via `react-native-razorpay` (`POST /api/v1/checkout/create-order` + `POST /api/v1/checkout/verify`); file download + share via expo-file-system v56 `File.downloadFileAsync` + expo-sharing; `useProducts` + `usePurchase` hooks; `ProductCard` component; `browse/products.tsx` + `browse/products/[slug].tsx` screens; browse `_layout.tsx` extended with 2 new Stack.Screen entries; `Product`/`CheckoutOrderResponse`/`CheckoutVerifyResponse` types + `productsApi`/`checkoutApi` namespaces added to `mobileApi.ts`; `EXPO_PUBLIC_RAZORPAY_KEY_ID` env var documented in `.env.example`; **Note: requires EAS development build (not Expo Go)** — Razorpay native module; tsc ✓ zero errors |
| **CDP Mobile Full Parity — users/funnels/cohorts/segments/analytics** (2026-06-24) | **done** — 8 root causes fixed in `services/api/app/modules/cdp/service.py`: (1) `_touch_user_trait` helper added; called from `log_event` + `batch_log_events` → every mobile event now creates/updates a `UserTrait` row; mobile users visible in `/admin/cdp/users`. (2) `batch_log_events` rows now include `platform`/`app_version` fields — offline-flushed mobile events no longer lose platform. (3) `FUNNEL_TEMPLATES`: `trek_view`→`trek_viewed` × 5, `trek_search`→`search_performed` — funnels now count both web + app events. (4) `get_trek_analytics`: `count_ev("trek_viewed")` (property-based, covers mobile) as primary; URL fallback for legacy web events. (5) `get_cohort_heatmap`: SQL rewritten from `analytics_sessions` base → `analytics_events` base — mobile users included in retention cohorts. (6) `SEGMENTS`: "Mobile-First Users" (device_type=mobile) split into "App Users" (platform_in=[ios,android]) + "Mobile Browser Users"; `platform_in` handler added to `get_segments`; 10→11 segments. (7) `get_user_activity`: `platform`/`app_version` fields added to event timeline output. (8) `get_content_pages_analytics`: adds `trek_viewed` property-based count for trek pages on top of `page_view` URL count — mobile trek views now included. `test_cdp.py` + `test_cdp_step65.py` segment count assertions updated 10→11. 719/721 backend tests pass (2 pre-existing); next build ✓ |
| **CDP Analytics Parity Fix — event names + platform filter** (2026-06-24) | **done** — Gap 1: `apps/mobile/lib/analytics.ts` — renamed `trek_view`→`trek_viewed`, `search_query`→`search_performed` to match web CDP event taxonomy; added `trackUserSignedIn(method)` (`conversion/user_signed_in`), `trackUserSignedUp(method)` (`conversion/user_signed_up`), `trackTrekShared(slug, method)` (`engagement/trek_shared`), `trackNewsArticleViewed(slug, title)` (`engagement/news_article_viewed`); `hooks/useAnalytics.ts` exports all 4 new helpers; `providers/AuthProvider.tsx` fires `trackUserSignedIn("email"/"google")` on signIn/signInWithGoogle and `trackUserSignedUp("email")` on signUp. Gap 2: `services/api/app/modules/cdp/service.py` — `get_events_explorer` + `get_events_export_csv` both take `platform: Optional[str]` param; `get_events_explorer` response dict now includes `platform` + `app_version` fields per event; `services/api/app/api/routes/cdp.py` — `GET /events` + `GET /events/export` both accept `platform: Optional[str] = Query(None)`; CSV export header/rows include `platform` column; `apps/web-next/app/(admin)/admin/cdp/events/page.tsx` — `platform` state added; platform filter dropdown (All / web / ios / android) in filter grid; `EventItem` interface extended with `platform`/`app_version`; Platform column added to table header + coloured badge (blue=web, purple=ios, green=android); colSpan bumped 6→7; export and buildQuery pass `platform`. 719/721 backend tests pass (2 pre-existing); tsc ✓ 0 errors; next build ✓ |
| **Step M16 — Trek Check-ins & History** (2026-06-24) | **done** — Alembic `20260623_0048_user_trek_history` (`user_trek_history` table: id/user_id/trek_slug/trek_title/completion_date/duration_days/rating/notes/trek_state/max_altitude_ft/extra/created_at + 3 indexes); `UserTrekHistory` ORM in `modules/mobile/models.py` + `db/base.py` registration; `CheckinIn`/`CheckinOut`/`TrekHistoryStatsOut` Pydantic schemas in `schemas/mobile.py`; service layer: `create_checkin`, `get_user_history`, `has_user_done_trek`, `get_history_stats` + 6 badge rules (`First Trek`, `5-Trek Club`, `10-Trek Veteran`, `Himalayan Explorer`, `Monsoon Warrior`, `High Altitude Ace`) in `modules/mobile/service.py`; 4 new API routes on `/api/v1/mobile/checkin`: `POST` (201), `GET` (history list), `GET /stats`, `GET /done/{slug}` in `routes/mobile.py`; 8 new tests TC-B-M16-01–TC-B-M16-08; 719/721 pass (2 pre-existing refresh ordering failures). Mobile: `hooks/useCheckin.ts`; `components/account/TrekHistoryCard.tsx` (date/rating/duration/state/altitude chips); `components/account/CheckinSheet.tsx` (Modal bottom sheet, star picker, date/duration/notes fields, success flash); `app/(tabs)/account/history.tsx` (stats row + badge pills + FlatList with pull-to-refresh + empty state); `app/(tabs)/account/_layout.tsx` — `history` Stack.Screen added; `AccountDashboard` — "Trek History" menu row added; `app/(tabs)/(home)/trek/[slug].tsx` — `useCheckin.isDone` on mount, "I did this trek — log it" CTA + "✓ You've done this trek" banner wired; `AuthProvider.tsx` — `setUserId` on login/logout (M15 wire-up completing). tsc ✓ 0 errors; next build ✓ |
| **Step 79 — Trek Buddy Matching (shared backend + web surfaces)** (2026-06-25) | **done** — Alembic `20260625_0050_buddy_matching` (`bio`+`avatar_url` added to `user_profiles`; `buddy_signals`, `buddy_requests`, `buddy_chat_messages` tables + 6 indexes + 2 unique constraints); `BuddySignal`/`BuddyRequest`/`BuddyChatMessage` ORM in `modules/buddies/models.py` + `db/base.py` registration; `SignalIn`/`SignalOut`/`BuddyCountOut`/`BuddyRequestIn`/`BuddyResponseIn`/`BuddyRequestOut`/`ChatMessageIn`/`ChatMessageOut`/`TrekkerProfileOut` Pydantic schemas; service: `_display_name` (privacy masking "FirstName L."), `_expires_at_for_month` (last day + 30 days), `get_buddy_count`, `get_trekker_profile`, `list_signals_for_trek`, `create_or_replace_signal` (upsert), `deactivate_signal`, `expire_signals`, `send_request`, `respond_to_request`, `get_chat_messages` (auto-mark read), `send_chat_message`, `mark_messages_read`; 3 router groups: `public_router` (`GET /public/treks/{slug}/buddy-count`, `GET /public/trekkers/{signal_id}`), `auth_router` (10 routes: signals CRUD, requests received/sent/respond, chat read/list/post — static routes before dynamic per §16); Celery beat task `buddies.expire_signals` (daily 86400s) registered in `celery_app.py`; 12 backend tests TC-B-M18-01–TC-B-M18-12, 739/741 pass (2 pre-existing). Web: `lib/buddies.ts` (TypeScript interfaces + API functions); `components/trek/BuddySignalCard.tsx` (privacy-safe card + inline connect composer); `components/trek/BuddySignalForm.tsx` (controlled form: month/group-size/experience/notes); `components/trek/BuddySection.tsx` (useAuth-gated section, count display, month breakdown chips); `components/trek/BuddyChatPanel.tsx` (10s polling, send/receive bubbles); `app/(public)/account/buddy-requests/page.tsx` (received/sent tabs, accept/decline, inline chat); `app/(public)/trekker/[signalId]/page.tsx` (public privacy-safe profile, no email/user_id); `/trek/[slug]/page.tsx` wired with `<BuddySection>`; account `layout.tsx` "Buddy Requests" nav item added. `next build` ✓ zero TypeScript errors (199 pages). 13 new URLs added to URL_MAP |
| **Step 80 — Live Trek Conditions (backend + web)** (2026-06-26) | **done** — Alembic `20260626_0051_trek_conditions_and_coords` (`trek_base_lat`/`trek_base_lng` Float nullable on `cms_pages`; new `trek_conditions` table: id/slug unique+indexed/weather_json JSONB/trail_status/permit_status/permit_notes/condition_summary/weather_updated_at/trail_updated_at/last_updated_at/created_at/updated_at); `CMSPage` ORM updated with `trek_base_lat`/`trek_base_lng` mapped columns; `modules/conditions/` module: `TrekCondition` ORM + `db/base.py` registration; `WeatherOut`/`ForecastDayOut`/`ConditionOut`/`SeedCoordinatesOut` Pydantic schemas; service: `TREK_COORDS` dict (40 Himalayan trek coordinates), `WMO_LABELS` mapping (Open-Meteo weather codes → label/icon), `fetch_weather` (async httpx to `api.open-meteo.com`), `_parse_weather`, `derive_trail_status` (last 5 approved trip reports majority vote + `trek_is_unsafe_closed` hard override), `derive_permit_status` (trek_permit_required + current month in trek_open_months), `build_condition_summary`, `refresh_trek_conditions` (upsert), `get_trek_conditions`, `refresh_all_trek_conditions`, `seed_trek_coordinates`; API routes: `GET /api/v1/public/treks/{slug}/conditions`, `POST /api/v1/admin/conditions/{slug}/refresh`, `POST /api/v1/admin/conditions/seed-coordinates`; Celery task `conditions.refresh_all` + beat schedule (every 6 hours, 21600s); 9 backend tests TC-B-M19-01–09, 748/750 pass (2 pre-existing refresh ordering failures). Web: `lib/conditions.ts` (TypeScript interfaces + `fetchConditions` with ISR `revalidate:3600`); `components/trek/LiveConditionsWidget.tsx` (current temp + WMO icon, humidity/wind/feels-like, 3-day forecast cards, trail status pill, permit pill, condition summary, offline-graceful); `trek/[slug]/page.tsx` wired with `<LiveConditionsWidget>` above trail-conditions section. `next build` ✓ zero TypeScript errors |
| **Step 80b — Live Conditions Admin Panel** (2026-06-26) | **done** — `ConditionAdminRow`/`ConditionsListOut` added to `modules/conditions/schemas.py`; `list_all_trek_conditions` service function (join published trek_guide pages + trek_conditions, coords_seeded flag from DB col OR TREK_COORDS dict, weather_label from weather_json.current.weather_code); `GET /api/v1/admin/conditions` + `POST /api/v1/admin/conditions/refresh-all` (Celery task dispatch via `apply_async()`) added to `routes/conditions.py`; static routes reordered before dynamic `/{slug}/refresh` per §16; `lib/admin-conditions.ts` TypeScript interfaces + 4 cookie-auth fetch fns; `app/(admin)/admin/conditions/page.tsx` — KPI cards (total/seeded/refreshed), "Seed Coordinates" + "Refresh All" action buttons, per-row "Refresh" button, coords status badge + trail/permit badges + last-refreshed relative time + first-time onboarding callout; `layout.tsx` "Live Conditions" nav item (CloudSun icon) under System. `next build` ✓ zero errors |
| **TrekSage enhancements — conversation logs, rate limiting, topic guard, CMS trek filters (2026-07-02)** | **done** — 3 enhancements: **(1) TrekSage conversation transcript viewer**: `SessionMessageOut`/`SessionTranscriptOut` schemas added to `schemas/trek_intelligence.py`; `get_session_transcript(db, session_key)` service function (joins `TreksageChatSession` → `TreksageChatMessage` → `User` for email/name); `GET /api/v1/admin/treks/ai-logs/session/{session_key}` static route in `admin_treks.py` (before dynamic `/{slug}/` routes per §16); `SessionTranscriptOut` imported; `session_id` field added to `AIInteractionLogResponse`; `list_ai_interaction_logs` propagates `session_id`; `apps/web-next/app/(admin)/admin/treksage-logs/page.tsx` — "Conversation" column added with "View chat →" link; `apps/web-next/app/(admin)/admin/treksage-logs/[session_key]/page.tsx` NEW — session meta card (user email/name/ID, created/last-active timestamps) + chat bubble transcript UI (user right-aligned accent, assistant left-aligned pine). **(2) TrekSage rate limiting + topic guard**: `check_daily_limit()` (Redis DB 3, 10 msgs/day anon, 30/day auth, key `treksage:rl:{YYYY-MM-DD}:{id}`, 90000s TTL, fail-open, `_BYPASS_IPS` for local/test); `is_off_topic()` (fail-open: no trek keyword + off-topic signal → reject without Claude call); rate limit + topic checks wired in `POST /api/v1/treksage/chat` handler; `except HTTPException: raise` added before generic except to prevent swallowing 429s. **(3) Master CMS trek filters**: `trek_permit_required: bool | None` added to `CMSPageResponse` + `CMSPage` interface; 8 client-side filter controls in `admin/cms/page.tsx` (trek_state, difficulty, duration buckets, permit_required, published_at from/to, updated_at from/to); all filter logic in `visiblePages` useMemo — no backend changes; `next build` ✓ zero errors. |
| **Mobile App UI/UX Pro Max Overhaul + Audit (2026-07-09)** | **done** — Comprehensive M01–M20 audit delivered (see artifact). Design overhaul: **(1) TrekCard** completely rebuilt — full-bleed portrait card (196×260px), 4-stop LinearGradient overlay, title/state/duration overlaid at bottom of image, difficulty dot-badge, shadow, expo-image fallback asset. **(2) HomeHero** expanded to 310px, auto-cycling 4-image carousel (5s interval, expo-image crossfade), seasonal context badge (month-aware), integrated frosted-glass search bar, saffron progress dots. **(3) CustomTabBar** — added visible text labels (10px) below all tab icons; TrekSage FAB gets "TrekSage" label; focused/unfocused color applied to labels. **(4) HomeTrendingSection** — featured first card (full-width 200px with gradient + Featured badge + CTA), remaining cards horizontal scroll (190×255px), "See all" link, trek count metadata under heading. **(5) PersonalisedFeedSection** — magazine layout: full-width hero card (190px), 2-column image-overlay grid for remaining; all images have LinearGradient overlay; location/difficulty chips on image. **(6) Bug fix — results.tsx:142**: `/trek/${slug}` → `/(tabs)/(home)/trek/${slug}` (trek cards in Plan Results were navigating to non-existent route). **(7) Bug fix — treksage.tsx**: same wrong route fixed for TrekSage "View Trek" buttons. **(8) TrekSage dark mode**: hardcoded `CREAM` background replaced with `colors.background` from `useTheme()`; input bar uses `colors.surface` in dark mode. **(9) PlanResultCard**: switched from `react-native Image` to `expo-image` + `LinearGradient`; real mountain fallback replaces ⛰️ emoji placeholder. `npx tsc --noEmit` ✅ zero errors. |
| **Mobile Audit Follow-up Bug Fix Pass (2026-07-09)** | **done** — 6 bugs fixed across 7 files based on the M01–M20 UI/UX audit: **(1) Plan results "Talk to operator" routes to step-6 loop**: `plan/results.tsx:148` changed `/(tabs)/plan/step-6` → `/(tabs)/plan` — users who already submitted the lead form were sent back mid-flow; now sends to plan intro. **(2) `handleRefresh` spinner race**: `(home)/index.tsx:61` `refetch()` wrapped with `await` — spinner disappeared before data loaded. **(3) Compare pill image `borderRadius: 0` inside round chip**: `compare.tsx` `pillImage` style corrected to `borderRadius: 20` — sharp image corners inside rounded pill container. **(4) REGIONS array duplicated in two files**: extracted to `constants/regions.ts` as canonical `Region[]` with exported union type; `browse/index.tsx` and `components/home/RegionsRow.tsx` now import from it. **(5) TrekSage `fontFamily` absent**: all 22 text styles in `treksage.tsx` StyleSheet had no `fontFamily` declaration — added `Inter_700Bold`/`Inter_600SemiBold`/`Inter_400Regular`/`PlayfairDisplay_700Bold` per font weight; system default font was rendering on the flagship AI screen. **(6) TrekSage thinking dots not animated**: replaced static opacity values with `Animated.Value` per dot + `Animated.loop(Animated.sequence(...))` with staggered 175ms delays — dots now pulse in sequence during loading. `Animated` added to react-native imports. `npx tsc --noEmit` ✓ zero errors. |
| **Mobile Bug Fix Pass 2 — 6 UX bugs from screenshot review (2026-07-13)** | **done** — **(1) Duplicate search bar removed**: `HomeSearchBar` removed from `(home)/index.tsx` (import + both render sites); hero search `onPress` updated to `/(tabs)/browse/search`. **(2) Hero carousel crossfade**: `HomeHero.tsx` — `Animated.Value` per image + `crossfadeTo()` via `Animated.parallel` (900ms, `useNativeDriver:true`); `Animated.View` wrappers replace bare Views on image stack. **(3) Operators back button fixed**: `canGoBack() ? router.back() : router.push("/(tabs)/browse")` — handles cross-tab navigation where browse stack has no prior entry. **(4) Table column truncation fixed**: `TableBlock.tsx` complete rewrite — fixed `width: 160` cell Views inside horizontal `ScrollView` with `flexShrink: 0` wrapper; text wraps within column instead of truncating. **(5) Shortlist icon fixed**: `TrekStickyBar.tsx` — `router.push("/(auth)/sign-in")` replaced with `Alert.alert("Sign in to save", ..., [Cancel, Sign In])` — user stays on trek detail page. **(6) Compare auto-scroll**: `compare.tsx` — `scrollViewRef` on outer `ScrollView`; `compareOffsetRef` via `onLayout` on table `View`; `useEffect` scrolls 300ms after `compareData` arrives. `npx tsc --noEmit` ✓ zero errors. |
| **Mobile Bug Fix Pass 5 — SafeAreaProvider root cause + trek detail robust redesign (2026-07-13)** | **done** — **ROOT CAUSE (Dynamic Island collision + broken navigation):** the app had **no `SafeAreaProvider`** anywhere, yet ~6 components call `useSafeAreaInsets()`. Without the provider the hook returns undefined insets → in `TrekHero`, `height + insets.top` = `380 + undefined` = **NaN** → RN drops the height → **hero collapsed to content height**, pinning title/state-pill under the Dynamic Island; the collapsed child also broke the ScrollView `stickyHeaderIndices` offset math → **tap hit-testing desynced ("clicks go to the wrong section")**. It also silently no-op'd the Pass-3 LiveConditions and Pass-4 sticky-inset fixes (insets were 0). **Fix (1):** added `<SafeAreaProvider initialMetrics={initialWindowMetrics}>` at the root of `app/_layout.tsx` — `useSafeAreaInsets()` now returns correct values app-wide (fixes hero buttons, LiveConditions header, TrekStickyBar, AppDrawer, sticky tab bar). `TrekHero` hardened with `topInset = insets.top ?? 0`. **Fix (2) trek detail nav bug:** removed the fragile `stickyHeaderIndices` + dynamic-height sticky tab bar; `TrekTabBar` now renders **inline** (constant height) plus a **pinned overlay copy** rendered OUTSIDE the ScrollView (absolute, `zIndex 20`, `topInset=insets.top`) that appears once the inline bar scrolls under the notch — no sticky-height change → no touch desync. **Fix (3) padding:** trust/safety/check-in block given `paddingTop:14` + `gap:10` breathing room (the cramping was the collapsed-hero side effect). **Fix (4) beginner cards 1-per-row:** `TrekCard` had a hardcoded `marginRight:14` (for horizontal scrollers) that overflowed the 2-col grid → added optional `noMargin` prop (default false, existing scrollers unaffected); `beginner.tsx` grid uses `Math.floor` card width + `noMargin` → 2 cards per row. Design authority: `.claude/skills/mobile-design-system/SKILL.md` (no separate "UI UX Pro Max" skill exists in repo). `npx tsc --noEmit` ✓ zero errors; Build Succeeded on iPhone 17 Pro. Frontend-only — no backend/deploy needed. |
| **Mobile Bug Fix Pass 4 — auth systemic fix + trek detail redesign (2026-07-13)** | **done** — **(1) Save trek 401 (systemic mobile auth)**: root cause — `get_current_user` (`app/modules/auth/dependencies.py`) validated ONLY the web session cookie (`typ=access`), but mobile sends `Authorization: Bearer` (`typ=mobile_access`); confirmed live — `POST /account/bookmarks/by-slug` with a Bearer token returned `{"detail":"Authentication required."}` (cookie-path message). **11 route files** use cookie `get_current_user` (account, subscriptions, plan, reports, buddies, checkout, monetization, …) so ALL were 401 from mobile. Fix: `get_current_user` rewritten to accept EITHER a Bearer `mobile_access` token OR the web cookie (strict superset — web unchanged, additive `_user_from_bearer` helper). Zero route/test churn (overrides target the same symbol). 3 new tests TC-B25/26/27 (bearer authenticates on account route + by-slug save + bad token still 401). **(2) Hamburger shows Sign Out when logged out + `user` null after cold start**: root cause — iOS Keychain (expo-secure-store) survives app reinstall, so `authStore.loadStoredToken` restored a stale token → `isAuthenticated=true` but `user=null` (never re-fetched); drawer showed Sign In buttons (`isAuthenticated && user`) AND Sign Out (`isAuthenticated`) at once. Fix: `AuthProvider.boot` now validates the restored token via `getMe` and rehydrates `user` (or `clearAuth` on failure); `AppDrawer` Sign Out gated on `isAuthenticated && user`. This also systemically fixes Bug 4 (sign-in/out consistency app-wide — save/buddy/check-in CTAs). **(3) Trek detail sticky tab bar hid behind Dynamic Island**: `TrekTabBar` gains a `topInset` prop applied only while pinned; `[slug].tsx` tracks scroll offset (`onScroll` + tab bar `onLayout`) to add `insets.top` padding exactly when stuck (no gap in normal flow). **(4) Beginner screen blank**: mobile queried only `beginner_guide` page type (0 pages live); web falls back to easy `trek_guide` pages (19 live) + editorial content. `beginner.tsx` rewritten to mirror web — fetches published trek_guide, filters easy/beginner, renders TrekCards + "11 mistakes" + "best first treks by city". **(5) Editorial card path** (from Pass 3) + **(6) Trek detail redesign**: `TrekHero` redesigned — cinematic 5-stop gradient, location state pill, dramatic title, and an at-a-glance stats panel (duration/altitude/difficulty with colour) overlaid on the hero; standalone `TrekMetaStrip` removed from the screen (facts now in hero → less cognitive load, child/sticky index preserved). Backend: `alembic upgrade head` applied pending `20260702_0053` (treksage anonymous_id) to sync local DB. `npx tsc --noEmit` ✓ zero errors; Build Succeeded on iPhone 17 Pro; 756 backend tests pass (2 pre-existing `test_refresh` order-isolation failures unrelated to this change — pass in isolation). **⚠️ Bug 1 fix requires production deploy** (mobile app targets `api.trekyatra.co.in`). |
| **Mobile Bug Fix Pass 3 — 6 UX bugs from screenshot review (2026-07-13)** | **done** — **(1) Trek buddy sign-in nudge**: `trek/[slug].tsx` buddy section — `{user && ...}` replaced with `{user ? ... : <TouchableOpacity onPress sign-in CTA "Sign in to find a trek buddy →">}` — non-authenticated users now see an actionable CTA instead of empty space. **(2) LiveConditionsScreen distorted + no back**: `LiveConditionsScreen.tsx` — added `import { useSafeAreaInsets } from "react-native-safe-area-context"`, expanded `header` style from `padding: 16` shorthand to individual sides; `paddingTop: insets.top + 16` applied inline so header starts below Dynamic Island on iPhone 17 Pro (Dynamic Island offset = insets.top ≈ 59px). **(3) Tab bar hidden on scroll**: `TrekStickyBar.tsx` — removed outer `<View style={{ paddingBottom: insets.bottom + 8 }}>` wrapper + `StyleSheet.absoluteFill` on `GlassSurface`; `GlassSurface` is now the direct container (`rounded="none" bordered={false}`) with children inside `<View style={[styles.bar, { paddingBottom: insets.bottom > 0 ? insets.bottom : (Platform.OS === "ios" ? 20 : 8) }]}>` — eliminates `GlassView` absoluteFill rendering bleed outside frame bounds on iOS 26 Liquid Glass. **(4) 2-column table extra right whitespace**: `TableBlock.tsx` — adaptive layout: if `numCols <= 2`, no horizontal `ScrollView`, cells use `flex: 1` (fills available width exactly); if `numCols >= 3`, retains horizontal `ScrollView` with fixed `COL_WIDTH = 160` — 3-column tables unchanged. **(5) Editorial "start here" card navigates wrong**: `EditorialFeatureCard.tsx` — `router.push("/beginner")` → `router.push("/(tabs)/(home)/beginner")` — unambiguous path resolves to correct screen in Home stack. **(6) Operators back button cross-tab**: `operators.tsx` — `router.canGoBack() ? router.back() : router.push(...)` replaced with `router.navigate("/(tabs)/browse")` unconditionally — `canGoBack()` returns true even for cross-tab global history, causing `router.back()` to return to Home; `navigate()` always switches to Browse tab root. `npx tsc --noEmit` ✓ zero errors; Build Succeeded on iPhone 17 Pro. |
| **Mobile Bug Fix Pass — trek card badge, table columns, compare markdown (2026-07-13)** | **done** — 3 bugs from user screenshot review: **(1) TrekCard difficulty badge invisible on bright images**: `difficultyColor()` default was `rgba(255,255,255,0.55)` (semi-transparent white); badge `backgroundColor: diffColor + "22"` = 8% opacity — invisible on any image. Fixed: `difficultyColor` now defaults to amber `#f59e0b` (always visible), handles "Moderate-Difficult" CMS compound string; badge `backgroundColor` changed from translucent tinted to solid `rgba(0,0,0,0.62)` dark pill — readable on any image; coloured dot still uses `diffColor`. **(2) Costs section table columns merged/stacked**: `HtmlContentRenderer` passed HTML directly to `react-native-render-html`; RNRH renders `<td>` as stacked blocks (no native table layout). Fixed: `HtmlContentRenderer.tsx` now splits HTML at `<table>` boundaries using `splitByTables()` — non-table HTML renders with RenderHTML as before; table segments are parsed with `parseTableHtml()` (handles `<thead>/<tbody>/<th>/<td>`, falls back to first `<tr>` as headers) and rendered with the native `TableBlock` component. **(3) Compare AI summary shows raw `**` markdown**: `compareData.ai_summary` is LLM-generated markdown rendered in a plain `<Text>`. Fixed: `MarkdownText` component added to `compare.tsx` — splits text on `\*\*...\*\*` pattern, renders matching spans as `fontWeight:"700"` bold `<Text>`, non-matching spans as plain text; used in place of raw `<Text>` for `ai_summary`. `npx tsc --noEmit` ✓ zero errors. |
| **Mobile App Bug Fix Pass — 7-bug resolution + UI/UX overhaul (2026-07-09)** | **done** — 7 bugs fixed in `apps/mobile/`: **(1) Plan wizard → auth → results broken**: `mobileApi.ts` `getAccessToken` now prefers Zustand in-memory token over SecureStore async read (eliminates post-login race condition); `results.tsx` now gates on `isAuthenticated && accessToken` (not just `isAuthenticated`); auth error in `fetchResults` gracefully falls back to re-login prompt instead of showing generic error screen. **(2a) About TrekYatra — Home tab tap does nothing**: `CustomTabBar.tsx` imports `router` from expo-router; when the home tab is already focused, `router.navigate("/(tabs)/(home)")` is called to pop the inner stack back to root (previously `!isFocused &&` guard prevented any navigation). **(2b) Emoji logo on About screen**: `about.tsx` imports `Image`; replaces `<Text>🏔</Text>` with `<Image source={require("@/assets/logo.png")} style={styles.logoImage} />` (80×80 `contain`); `SafeArea edges` changed from `["bottom"]` → `["top","bottom"]`. **(3) Trek hero image broken for treks with no `hero_image_url`**: `CMSPage` TS interface extended with `route_image_url?: string | null`; `trek/[slug].tsx` passes `trek.hero_image_url ?? trek.route_image_url ?? null` to `TrekHero`. **(4+7) Trek detail page premium redesign**: `TrekHero.tsx` rewritten — default height 300→360px, stronger 4-stop gradient, saffron state pill, text-shadow on title, back button with `router.back()` (circular dark pill), optional `onShare` prop renders share pill top-right; header now `headerShown: false` in `_layout.tsx` (no duplicate header); share button moved inside `TrekHero`. `TrekMetaStrip.tsx` rewritten — chips now have square icon wrapper (Ionicons, colored background), card-style shadow elevation, larger hit target (8px gap). **(5) Tables breaking (flex layout)**: `TableBlock.tsx` fully rewritten with `StyleSheet.create()` (removes NativeWind className — all rows now explicitly `flexDirection: "row"`, `minWidth: 120` per cell, alternating row background, horizontal `ScrollView`). **(6) Shortlist icon not working**: `TrekStickyBar.tsx` — heart icon replaced with `Ionicons bookmark` (filled on save); save error now shows `Alert.alert` instead of silent catch; compare button route corrected from `/compare?slug=…` → `/(tabs)/(home)/compare?slug=…`. `tsc --noEmit` ✓ zero errors; build succeeded on iPhone 17 Pro simulator. |
| **Route image field — CMS, frontend, schema, sitemap (2026-06-30)** | **done** — New `route_image_url VARCHAR(512)` column on `cms_pages`: **(1) Migration** `20260630_0052_cms_route_image_url.py` applied; **(2) ORM** `CMSPage.route_image_url` mapped column added; **(3) Pydantic** `CMSPagePatch` + `CMSPageResponse` extended with `route_image_url`; **(4) `lib/api.ts`** — `CMSPage` interface + `CMSPagePayload` extended; **(5) `buildTrekSchema`** (`lib/schema.ts`) — `routeImageUrl` param added; when both hero + route images exist emits `image` as array of two `ImageObject` entries (`name: "Trail Route Map"` on route); **(6) `CMSPageForm.tsx`** — dedicated "Route image" card (trek_guide only): URL input + file upload (reuses `/api/v1/admin/media/upload`) + live preview with Remove button; **(7) `trek/[slug]/page.tsx`** — route image displayed below Route Overview text (`not-prose mt-6` div, `loading=lazy`); `routeImageUrl` passed to `buildTrekSchema`; **(8) Sitemap + hero "Updated at"** are automatic — `TimestampMixin.updated_at` has `onupdate=func.now()` so any PATCH saving `route_image_url` bumps `updated_at`, which flows into `effectiveUpdatedAt` (hero badge) and `GREATEST(updated_at, conditions.last_updated_at)` (sitemap) with zero extra code. 54/54 CMS tests pass; `next build` ✓ zero errors. |
| **Trek page: effective updated date + conditional JSON-LD conditions schema (2026-06-29)** | **done, TC-F01–TC-F04 confirmed on prod (2026-06-30)** — Two enhancements to `apps/web-next/app/(public)/trek/[slug]/page.tsx`: **(1) Effective updated date**: `TrekDetailPage` now fetches `GET /api/v1/public/treks/{slug}/conditions`, `GET /api/v1/public/treks/{slug}/reports`, and `GET /api/v1/public/treks/{slug}/buddy-count` in parallel at server render time (`Promise.allSettled`, best-effort). `effectiveUpdatedAt = GREATEST(cmsPage.updated_at, conditions.last_updated_at)` — the hero badge "UPDATED …" and both `articleSchema.updatedAt`/`trekSchema.updatedAt` now use the effective date so Google re-crawls the page whenever conditions refresh. **(2) Conditional JSON-LD schema**: the 3 `additionalProperty` entries (Live Trail Conditions / Community Trail Reports / Trek Buddy Matching) are now only added when data exists (conditions row with `last_updated_at` set, reports `total > 0`, buddy `count > 0`) AND carry real values: conditions shows `condition_summary` or `"Trail {status} — {temp}°C, {label}"`, reports shows `"N verified reports"`, buddy shows `"N trekkers looking for a buddy"`. If all three are empty the `additionalProperty` override is omitted entirely. `npx tsc --noEmit` ✓ zero errors; `next build` ✓ zero errors. |
| **M20 NearbyTreksSection UX fixes (2026-06-29)** | **done** — 3 issues from prod screenshot: **(1) Missing altitude/season on cards**: `NearbyTrekOut` Pydantic schema missing `trek_season`; backend route had `trek_altitude=None` hardcoded — fixed: altitude now extracted from `page.content_json["trek_facts"]["altitude"]`, season from `page.trek_season`; `NearbyTrekItem` TS interface and `toTrek()` mapper updated to use both fields. **(2) Distance badge overlapping bookmark icon**: removed absolute-positioned overlay badge on card; replaced with below-card label "X km from your location" with MapPin icon — no UI collision possible. **(3) Distance context ambiguous**: section subtitle changed to "Distances from your current location · showing results within 300 km"; per-card label says "X km from your location" not bare "X km". Mobile `mobileApi.ts` `NearbyTrekOut` interface updated with `trek_season`. 5/5 tests pass; `next build` ✓ clean. |
| **Step 81 — Clean comparison pages + publish-triggered comparison agent (2026-07-14)** | **done** — #8 of the home/CMS enhancement pass. New `modules/comparison/service.py` (deterministic, no LLM): `pair_slug` (canonical alphabetical `{a}-vs-{b}`, de-dupes both orderings), `trek_summary` (first-class columns + `content_json.trek_facts` fallback), `build_verdict` (rule-based picks: beginner/altitude/shorter/budget + summary sentence), `build_comparison_content` (7-row table + verdict), `generate_comparison_page` (idempotent upsert of a `page_type="comparison"` published CMS row), `_same_state_peers` (guards null `trek_state` to avoid combinatorial explosion — 5644 stateless seed pages), `generate_comparisons_for_trek` (same-state top-3), `backfill_all_comparisons`, `list_comparison_pages`. Celery tasks `comparison.generate_for_trek` + `comparison.backfill_all` (`worker/tasks/comparison.py`, registered in `celery_app.py`). Publish trigger: `PATCH /cms/pages/{slug}` dispatches `generate_for_trek.delay` when a trek_guide becomes published (guarded — down broker never fails publish). Admin routes `POST /admin/comparisons/backfill` + `/generate/{slug}` (`routes/comparison.py`, `get_current_admin`, registered in `router.py`). Sitemap auto-covers comparison pages via existing FE `PAGE_PREFIX["comparison"]="/compare"`. Web: `app/(public)/compare/[pair]/page.tsx` — server-rendered `{A} vs {B}`, `generateStaticParams` from published comparison pages, `generateMetadata` (canonical `/compare/{pair}`, OG), breadcrumb + Article JSON-LD, two trek columns + verdict card + comparison table + CTAs. 7 backend tests TC-B-C01–C07, 763 pass (2 pre-existing refresh failures). `next build` ✓ zero errors. E2E verified against dev DB (generated 3 same-state pages, canonical slug, published, 7 rows). Also in this pass (#1–#7, prior working tree): dynamic home regions via new `GET /public/trek-state-counts`, editorial spotlight from newest real guide, trek-detail quick-utility anchors, static trek stub removal (CMS sole source), mobile tab "Trail"→"Conditions", CMS admin route-image filter, conditions/data-quality ordering by `published_at desc`. |
| **PageSpeed Insights audit (mobile) — findings + fixes (2026-07-21)** | **in progress** — PSI mobile Performance **56** (LCP **12.8s**, Speed Index 12.5s, FCP 4.2s; CLS 0 + TBT 150ms fine → a *loading* problem). Root causes (code-grounded): **#1 TTFB 3.7s** — `app/(public)/page.tsx` is `force-dynamic` + 7 blocking `apiFetch` calls (all use `AbortSignal.timeout` → uncached + force route dynamic → `Cache-Control: no-store` → not cacheable at Next OR Cloudflare). **#2 Images ~4.7MB** — trek cards use plain `<img src={Spaces JPEG}>` (`TrekCard.tsx:82`), NOT Next/Image → raw 2.2MB uploads, oversized (1607×1814 vs 1093×732 display), JPEG not WebP/AVIF (Step 71 converted the hero only). **#3 Spaces `Cache-Control: None` (~5.2MB)** — Step 71 infra item (boto3 CacheControl + Cloudflare rules) still pending. **#4 React hydration errors #418/#423/#425** — SSR↔client mismatch (suspect `toLocaleDateString("en-IN")` editorial date, page.tsx:131); fails Best Practices + forces client re-render. **#5 3rd-party JS ~250KB** — Google Sign-In (`gsi/client` 95.8KB) + GTM (158.9KB) eager-ish. Minor: render-blocking CSS 610ms, Logo_Trekyatra.png 301KB, unused unsplash preconnect, legacy polyfills 11KB, 401 `/auth/me` console error, home `<select>` missing labels (a11y 91). **Implemented #1 + #2 (this change):** **#1** — `app/(public)/page.tsx`: removed `force-dynamic`, wrapped the 7 catalog fetches in `unstable_cache({revalidate:300, tags:["cms:all"]})` + `export const revalidate = 300` → route is now `○` Static/ISR; VERIFIED prod-mode: `Cache-Control: s-maxage=300, stale-while-revalidate` (was `no-store`), `x-nextjs-cache: HIT`, ~5ms cached (was 3.7s TTFB), 0 static-to-dynamic errors. Personalisation stays client-side (`makeDynamic ssr:false`); CMS cache-clear's existing `revalidateTag("cms:all")` busts it. **#2** — `TrekCard.tsx`: `<img>` → `<Image fill sizes=…>` (Next optimizer → WebP/AVIF + resize + srcset); VERIFIED: 100 optimized `/_next/image` trek-card images on home. Remaining: **11 raw Spaces `<img>`** on home are *non-card* (editorial etc.) — follow-up. #3 (Spaces `Cache-Control` + Cloudflare rules) is DO/infra (Step 71 pending); #4 (hydration) / #5 (defer GSI) / minor are follow-ups. Note: prod lacks `sharp` → Next image optimizer uses the slower JS fallback (installing sharp would speed server-side optimization; client payload already fixed). **Implemented #3 + #5 + #4 hardening (this change):** **#3** — `services/api/app/api/routes/media.py`: `_upload_to_spaces` now sends `CacheControl: public, max-age=31536000, immutable` on upload (uuid4 keys are content-unique → immutable safe); new one-off `services/api/scripts/backfill_spaces_cache_control.py` rewrites the header on existing objects via `copy_object(MetadataDirective=REPLACE)` (idempotent, `--dry-run`/`--prefix`). **DO deploy step (run once after deploy):** api component Console → `cd services/api && python scripts/backfill_spaces_cache_control.py`. Test: `test_media_cache_control.py` (mocks boto3, asserts the header). **#5** — GSI (`gsi/client` ~95KB) was injected on EVERY page because `GoogleOAuthProvider` wrapped the whole app in `components/Providers.tsx`. New `components/auth/GoogleAuthButton.tsx` encapsulates the provider+hook+button; the global provider was removed and the 3 consumers (sign-in, sign-up, plan `AuthGateModal`) now use it. VERIFIED via `.next/app-build-manifest.json`: home `/(public)/page` and root `/layout` no longer load the GSI chunk; only sign-in/sign-up/plan/compare do. **#4** — the flagged suspect (editorial `toLocaleDateString`, page.tsx) is rendered in a **Server Component** (computed once server-side) so it does NOT itself cause a hydration mismatch; the home's client components (SeasonalTreks, HomeComparisons) already use constant defaults + post-mount effects, and auth-context starts `null` both sides — the home tree is hydration-clean. Still shipped defensive TZ/ICU-deterministic `apps/web-next/lib/date.ts` (`formatDate`, UTC getters + fixed months) applied to home/trek/news/news-detail (protects the client-rendered date spots + removes ICU drift). Residual #418/#423/#425 in the field are most likely third-party (GTM/AdSense/GSI DOM injection) or browser-extension noise — if they persist, next step is a non-minified repro to pinpoint. VERIFIED: `next build` clean, home ISR intact (HIT, s-maxage=300, 0 static→dynamic), 769 backend tests pass. Still pending: Cloudflare HTML cache rules + `sharp` on prod (infra); 11 non-card home `<img>`; minor a11y/polyfill items. **#4 investigation + follow-up (this change):** Reproduced the live #418/#423/#425 with headless Chromium — **real on live, NOT reproducible on identical code locally under any condition** (dev, prod build, deployed commit `d9ffb7f`, `TZ=UTC` server + IST client). **Ruled out** third-party (block-matrix of gsi/gtm/ga/adsense/doubleclick → errors persist), timezone, and locale-number formatting. `MutationObserver` (pre-script) recorded **zero DOM swaps** → the mismatch resolves to identical final output (benign structural/whitespace mismatch; hurts Best-Practices score + forces a boundary re-render, not a user-visible bug). Prod React emits no component stack (minified `pageerror` only), so pinpointing needs a non-minified prod signal. **Actions:** (a) fixed a genuine LATENT hydration bug — `components/layout/Footer.tsx` `© 2023–{new Date().getFullYear()}` (client component; static/ISR build year vs client year mismatches across New-Year) via `suppressHydrationWarning` (React's official timestamp escape hatch; keeps correct year in SSR, no flash, chosen over a mount-guard as the smaller/safer diff); (b) added **`productionBrowserSourceMaps: true`** to `next.config.mjs` — **⚠️ TEMPORARY, REVERT after debugging** — runtime JS/HTML unchanged (only emits `*.js.map`), so it cannot break live; lets the next deploy demap the minified frames to the exact component. VERIFIED: build clean, `.js.map` emitted, home ISR intact (HIT, s-maxage=300), 0 local hydration errors. **TODO (tracked): revert `productionBrowserSourceMaps` once the culprit node is identified from the source-mapped deploy.** **#4 CLOSED (this change):** Deployed the source-mapped build (`69f602f`) and re-ran the capture against live — the #418/#423/#425 stack frames are **pure React reconciler internals** (all in the framework chunk, no app frame), so source maps could not name the component and prod React emits no component stack. Further isolation: the live home renders a **real editorial spotlight + real trek data** that my local builds usually lacked (the `apiFetch` `AbortSignal.timeout(3000)` aborts the Mac→prod call → static fallback), which is why local never reproduced. Re-tested **dev mode with the real editorial path rendering** (UTC server + IST client, GA4/AdSense env set) → **0 warnings**. Conclusion: the errors reproduce **only** in the live production build (never dev or local-prod, with or without real data), cause **no DOM change** (benign), and `#423` is specifically a **Suspense-boundary** hydration recovery — the known-benign App-Router `next/dynamic({ ssr:false })` pattern (home uses two: `RecentlyViewedSection`, `PersonalisedFeed`). This is cosmetic (Best-Practices score only, no user-facing bug); a definitive component name is not obtainable without patching React to keep prod component stacks (not worth the prod risk). **Reverted `productionBrowserSourceMaps`** (`next.config.mjs`) — confirmed useless for naming + avoids exposing source/build bloat. **Kept:** Footer `suppressHydrationWarning` (real latent New-Year bug). #4 status: hardened + latent bug fixed; remaining warnings benign/accepted. **#3 BACKFILL EXECUTED + infra verified (2026-07-27):** ran `backfill_spaces_cache_control.py` on the DO api component Console → **83/83 media objects updated, 0 errors**; verified `cache-control: public, max-age=31536000, immutable` (Content-Type preserved). **Cloudflare follow-up = already satisfied** (verified live `cf-cache-status: HIT` on `/_next/static/*` [immutable], `/_next/image` optimized trek cards [now `max-age=31536000` — the Spaces backfill propagated the long TTL into the optimizer output, was 60s default], and home HTML [`s-maxage=300`]); the direct Spaces host isn't CF-proxied but doesn't need to be (immutable browser cache). **No CF cache rule required.** Optimizer confirmed working (187KB→53KB at w=828, 200 not 504 — an earlier 504 was a malformed test URL). **`sharp` — REASSESSED after a prod incident (2026-07-27):** originally called "optional", which was WRONG for this dyno. After `d513190` (logo optimized on EVERY page + 4 more home images), the web dyno began **thrashing** — live FE flapped between slow 200s (~1.5–3s, vs ~5ms cached) and intermittent **504s** on `/`, `/_next/image`, and even raw `/public` assets → images/logo appeared broken. Root cause: **no `sharp` → Next used the WASM/squoosh optimizer fallback (~10× heavier CPU/mem)**, and the added per-page logo optimization starved the dyno. **FIX `12c6ee0`:** added `sharp@0.33.5` (web `package.json` + root `package-lock.json` incl. all `@img/sharp-linux{,musl}-{x64,arm64}` binaries → DO Node buildpack installs the right one). Next auto-uses sharp when present → fast, low-memory optimization → no thrash; all image optimizations retained. Immediate mitigation available via DO dashboard rollback to pre-`d513190`. #3 fully banked; sharp now REQUIRED not optional. **#4 optional polish — mount-gate SHIPPED (2026-07-27):** new `components/util/ClientOnly.tsx` (renders children only post-mount → server + first client render emit nothing → nothing to hydrate); `app/(public)/page.tsx` wraps the two `ssr:false` sections (`RecentlyViewedSection`, `PersonalisedFeed`) in it so their `next/dynamic` Suspense boundary is created post-hydration instead of hydrated → clears the benign prod-only #418/#423/#425. VERIFIED: build clean (sharp still resolved), home ISR intact (HIT, s-maxage=300), 0 local hydration errors, adds NO optimizer load. NOTE: the warnings only reproduce in the live prod build (never locally), so the console-clear is confirmable only after deploy. **llms.txt (2026-07-27):** created `apps/web-next/public/llms.txt` (llmstxt.org format) — a curated markdown overview + key **public** URLs (start-here, treks, guides by type, regions/seasons, compare, news, operators, about, programmatic access via Datacenter/MCP/profile API, sitemaps) for AI/LLM consumers. Static public file → served at `/llms.txt` (text/plain, verified 200), zero code/route. Only public pages listed (no admin/account/auth); URLs sourced from `docs/URL_MAP.md` (no invented paths). Meta-file → NOT in sitemap. URL_MAP updated. **PSI re-audit + LCP fix (2026-07-27):** mobile PSI improved to **Performance 77** (was 56), FCP 1.7s (was 4.2), LCP **5.0s** (was 12.8), A11y 96, Best-Practices 96, SEO 100, Agentic Browsing 2/2. Diagnosed remaining mobile LCP: the hero image (`hero-himalaya-dawn.jpg`) is preloaded (`fetchPriority=high`) and tiny (26–32KB AVIF), but **local `/public` optimized images were served with `Cache-Control: max-age=60`** — Next's `minimumCacheTTL` default — because local images have no upstream Cache-Control to inherit (Spaces images inherit `immutable` from the backfill). At 60s, Cloudflare expired them constantly → REVALIDATE/MISS → origin re-optimize per load → slow LCP + needless dyno load. **FIX:** `next.config.mjs` `images.minimumCacheTTL: 31536000` → local optimized images now `max-age=31536000` (verified) → CF caches a year → HIT. `gitnexus detect-changes`: 2 symbols, 0 processes, risk LOW. Also flagged in the audit but NOT real code defects: **charset "error"** — the page correctly declares charset (HTTP header `charset=utf-8` + `<meta charSet>` at byte 105, within the 1024 limit); the Lighthouse flag was captured during the sharp incident's 504 window → re-run PSI to clear. **ChunkLoadError / RSC / ERR_TIMED_OUT** — Lighthouse Slow-4G throttling + client prefetch storm; my unthrottled headless run showed 0 (sharp fix held; site stable). **#418/#423/#425 STILL present (5)** — the mount-gate did NOT clear them; benign (no DOM change), prod-only, unreproducible locally; not pinned. **Missing source maps** — intentionally reverted (unscored/informational). **Infra follow-up:** add a Cloudflare cache rule for `/_next/image*` as belt-and-suspenders (the code fix handles the TTL; a rule ensures CF caches query-string image URLs). **Critical-path trims + mount-gate revert (2026-07-27):** (1) `app/layout.tsx` — `preload: false` on Fraunces (display) + JetBrains_Mono (mono) fonts (kept Inter/body preloaded) → font preloads 3→1, so those woff2 no longer compete with the LCP hero image on slow connections (`display:swap` handles the FOUT); (2) removed the `trekyatra-media.sgp1.digitaloceanspaces.com` preconnect — now unused because all images are served same-origin via `/_next/image` (the optimizer fetches Spaces server-side), Lighthouse flagged it; (3) **reverted the mount-gate** — `ClientOnly.tsx` deleted + the two home sections unwrapped, since it did NOT clear the #418/#423/#425 (dead code). `gitnexus impact RootLayout` = 0 processes/LOW; `context ClientOnly` = 0 incoming. VERIFIED: build clean, hero still preloaded (`fetchPriority=high`), home ISR intact. **Charset confirmed a non-issue** — Lighthouse 13.4 charset audit run against the LIVE URL scores **1 (PASS)**; the PSI failures coincided with the sharp-incident 504 window + the minimumCacheTTL mid-deploy cold-cache state. **Remaining PSI levers (tradeoffs, NOT auto-applied):** legacy-JS polyfills (~11KB, browserslist tighten — drops pre-2021 browser support); trek-card image quality q75→~q65 (~86KB, product/visual tradeoff); render-blocking CSS + unused CSS/JS (framework/Tailwind/GTM — mostly third-party or hard). **SEO fix — robots.txt sitemaps (2026-07-28):** `app/robots.ts` only listed 2 of 11 sitemaps and had a wrong `SITE_URL` fallback (`trekyatra.com`). Now lists all 11 (core + 7 state-trek + hi-trek + news + compare) with the correct `https://www.trekyatra.co.in` fallback — the core `sitemap.xml` is a urlset (not a sitemap index) so children weren't being discovered. `gitnexus impact robots` = 0 impacted, LOW. VERIFIED: `/robots.txt` renders 11 Sitemap lines, correct domain, build clean. (Follow-up option: the child-sitemap `<url>` entries in `sitemap.ts` are now redundant — could be removed since robots lists them directly.) **DONE that follow-up (`8989ae5`):** removed the child-sitemap `<url>` entries from `sitemap.ts` (they're a `<urlset>`, not a sitemap index; children live in robots.txt). **Mobile analytics instrumentation (2026-07-28):** M15 infra was built but events weren't firing — now instrumented. `providers/AnalyticsProvider.tsx`: auto `screen_view` on every route change (`usePathname`, dedup). `lib/analytics.ts`: every event now carries `device_model`+`os_version` in `properties` (installed `expo-device`). Wired interaction events: `trek_view`+`trek_shared` (`trek/[slug].tsx`), `trek_saved` (`TrekStickyBar`), `search` (`browse/search.tsx`, one event per settled query), `operator_inquiry` (`useOperators` mutation onSuccess — covers all call sites). `gitnexus impact usePurchase/usePremium`-style checks LOW; mobile tsc 0 errors. **Still to wire (follow-up):** `plan_wizard_step/completed`, `trek_downloaded`; `news_article_viewed` waits on M21 (mobile news screen not built); `push_opened` deferred with push; product/premium events are iOS-removed. Events flow to the SAME CDP (`/api/v1/analytics/event`) → verify in admin CDP Event Explorer filtered `platform=ios`. **PSI minor quick-wins batch (2026-07-27):** **DONE (4):** (1) 4 non-card home `<img>` → Next `<Image fill sizes>` (region cards, editorial spotlight [Spaces hero], resources, hero-CTA bg) — `app/(public)/page.tsx`; (2) removed unused `images.unsplash.com` preconnect — `app/layout.tsx` (kept Spaces preconnect); (3) `components/brand/Logo.tsx` `<img>` (308KB PNG rendered every page) → Next `<Image>` → served optimized WebP at 48/56px w/ srcset; (4) `aria-label` on the home search region+season `<select>`s — `components/home/HomeSearchBar.tsx` (a11y). VERIFIED: build clean, home ISR intact (HIT, s-maxage=300), logo + all home images via `/_next/image`, unsplash preconnect gone, both aria-labels present. **SKIPPED (3, with reason):** (a) **browserslist tighten** — deliberately NOT done: the ~11KB legacy polyfills are conditionally served and modern browsers skip them anyway, while tightening drops support for older Android/Safari devices that are a real slice of the India audience (product decision, negative trade); (b) **`401 /auth/me` console line** — already handled in JS (`getCurrentUser` catches → returns null); the remaining line is the browser's own network-error log which JS cannot suppress without a non-httpOnly login-hint cookie (risks auth-state desync — not worth it for a cosmetic log); (c) **render-blocking CSS** — framework-level (Next App Router controls CSS injection), not safely fixable without significant risk. |
| **Sitemap — separate news + compare child sitemaps (2026-07-21)** | **done** — News (`/news/{slug}`) and comparison (`/compare/{a-vs-b}`) URLs were being emitted **directly into the core `sitemap.xml`** (news via `PAGE_PREFIX.news_article`, compare via a pairs loop), duplicating/bloating the root. Now both live in **separate child sitemaps referenced from core** (matching the state-trek-sitemap pattern). Frontend-only, leaf routes (gitnexus MCP down → manual impact analysis; SEO-critical so output verified). Changes: NEW `app/compare-sitemap.xml/route.ts` (fetches `GET /public/comparisons`, emits `<url>` per `/compare/{pair}`); `app/sitemap.ts` — `PAGE_PREFIX.news_article` set to `undefined` (news now only in `/news-sitemap.xml`, which already lists all 106 news URLs), removed the direct compare-pairs loop + its fetch helper, added `url("/compare-sitemap.xml")` reference. Verified prod-mode: core references both child sitemaps, **0** direct `/news/` and **0** direct `/compare/` `<loc>` entries in core; `/compare-sitemap.xml` = 131 pairs, `/news-sitemap.xml` = 106 news, `/news` hub kept. `next build` ✓. |
| **Home page — news-out-of-trek-sections + Recent News section (2026-07-21)** | **done** — Web (desktop + mobile-web) home-page pass. Root theme: news articles leaked into trek-focused personalization (they were recorded as views when `/trek/{news-slug}` still rendered, before the 404 guard). All frontend, no backend change; impact = leaf home components (gitnexus MCP down → manual analysis). **(1)** `RecentlyViewedSection.tsx` ("Pick up where you left off") now filters `behavior-tracker` views to real treks only (slug ∈ trek catalog `trekList` or `cmsImageMap`) — news no longer shows as chips. **(2+4)** `PersonalisedFeed.tsx` ("Treks based on your browsing history", also on `/explore`) filters recommendations to `page_type==="trek_guide"` (fetches `limit*3` to compensate) and `FeedCard` always links to `/trek/{slug}` — fixes the wrong `/guides/{news-slug}` URLs; recommendations backend untouched (shared with the native app). **(3)** NEW `components/home/RecentNewsSection.tsx` (client) — published news grouped into **per-trek tabs** (label = trek name via `trek_slug→name` map, else slugified), newest→oldest, ≤5 per tab, a trek needs ≥1 article for a tab; "View all News Articles" → `/news`. Wired into `page.tsx` after PersonalisedFeed (added `fetchNewsArticles(60)` to the parallel fetch + `trekNameMap` from `trekList`). Verified: `next build` ✓; prod-mode ✓ (home 200, Recent News renders 24 trek tabs from 59/60 news with trek_slug, View-all present, **0 `/guides/{news}` links**, 0 errors). PersonalisedFeed/RecentlyViewed are client-rendered → validated by build + code. |
| **CMS bugfixes — auto-brief gate + news duplicate-URL (2026-07-21)** | **done** — **Bug 1 (auto-briefs burning tokens):** the `pipeline.daily_discovery` Celery beat task (daily) auto-ran trend-discovery → content-brief generation. Gated behind new `settings.enable_daily_discovery` (default False, `config.py` + `.env.example ENABLE_DAILY_DISCOVERY`); `daily_discovery_task` early-returns `{"status":"disabled"}` unless enabled — no LLM spend. Weekly news agent kept running (user decision). Needs prod **worker restart** to take effect. **Bug 2 (news article reachable at duplicate `/trek/{slug}`):** root cause = cross-step gap — the Step 56 weekly news agent creates `news_article` CMS pages, but the Step 44/50 linking module's `_page_type_from_cms` has no `news_article` case → defaulted to `"trek_guide"`, so `linking.sync_pages` imported news into the trek linking graph; they surfaced in the "In this cluster" sidebar with a `/trek/{slug}` URL, and `/trek/[slug]` had no page_type guard so it rendered the news article as a broken trek page (duplicate content). NOT a sitemap dup (sitemap maps by real page_type → `/news` only); harm is via internal-link crawl. Fixes: **(A)** added `news_article` to `_EXCLUDED_FROM_LINKING` (linking/service.py) — sync now purges news from the graph; **(B)** `/trek/[slug]/page.tsx` route guard — a published CMS page with `page_type !== "trek_guide"` never renders as a trek page: **`news_article` → 404 (the `/trek/{news-slug}` pattern is deleted per product decision; news is served ONLY at `/news/{slug}` as a direct 200, no redirect)**; other content types 308-redirect to their canonical prefix; **(C)** "In this cluster" URL builder now routes by page_type via a prefix map (never defaults non-trek to `/trek/`). One-off cleanup: `services/api/scripts/purge_news_from_linking.py` (or `POST /admin/links/sync`) removes already-synced news rows from the linking graph. **"In this cluster" reconciliation (follow-up):** cluster must show ONLY published trek links + news (as `/news/{slug}`). **(D)** `get_related_pages` now filters candidates by the **REAL CMS `page_type`** (join `Page→cms_pages`) so a mis-typed news row (typed `trek_guide` by the default mapping) can NEVER leak into a trek's related list — robust regardless of whether a re-sync/purge has run. **(E)** desktop `trek/[slug]/page.tsx` "In this cluster" = `clusterPages` filtered to `trek_guide` + this trek's news (`fetchNewsByTrek` → `/news/{slug}`). Mobile `RelatedPagesSection` left as-is (user deferred); it now gets a news-free cluster from the same endpoint but still routes other guide types to `/guide/` — desktop/mobile intentionally diverge for now. Tests: `test_get_related_pages_excludes_mistyped_news` (reproduces the leak + asserts exclusion), `test_news_article_excluded_from_linking_graph`, `test_daily_discovery_disabled_by_default` — all PASS (Docker/Postgres now up; 167 related tests pass, full suite 765 pass / 2 pre-existing). `next build` ✓; prod-mode smoke ✓ (`/trek/{trek}`→200, `/trek/{news-slug}`→**404**, `/news/…`→200, 0 errors). Deploy: FE+BE deploy + worker restart + run purge script / `linking.sync_pages` once. |
| **Home comparisons — server-synced cross-device personalization (2026-07-14)** | **done** — The home compare section's difficulty/region personalization was anonymous/local (localStorage only), so a logged-in user on a new device saw no personalization until they re-browsed. Fix reuses the existing cross-device sync (`auth-context` → `pullAndMergeBehaviorProfileFromBackend` on login/session-restore merges the server `behavior_profile` into localStorage) by making it observable: `lib/behavior-tracker.ts` now exports `BEHAVIOR_UPDATED_EVENT` and fires it after the server merge (and on each `recordTrekView`); `components/home/HomeComparisonsSection.tsx` reads the profile on mount **and** re-reads on that event (the async merge resolves after mount, so a one-shot read missed it) → cards re-rank by the cross-device most-viewed difficulty/region signal without a reload. Impact analysis LOW (behavior-tracker leaf; only AuthProvider upstream). `next build` ✓. Note: `PersonalisedFeed` is already server-driven (`fetchPersonalisedRecommendations` reads the server profile) — unchanged. |
| **Prod 500 fix + ISR caching restore (2026-07-14)** | **done** — Regression: PT4's CMS-backed `fetchTreks` fed real slugs into `generateStaticParams` on the `[slug]` CMS detail pages, pulling them into the ISR manifest; combined with `fetchCMSPage`'s pre-existing `cache:"no-store"` (Step 38) + `export const revalidate` (Step 43), Next threw "Page changed from static to dynamic at runtime" → **500 on every published trek page** (also guides/permits/packing + hi/*). **Emergency fix (effbef9):** `force-dynamic` on trek/[slug] + hi/{trek,guides,packing} (verified in `next start` prod-mode — the error only surfaces at runtime, never in `next build`). **Perf restore (this change):** `fetchCMSPage` switched from `no-store` → `next:{revalidate:60, tags:["cms:{slug}","cms:all"]}` (dropped `AbortSignal`, which opts a fetch out of Next's Data Cache) → CMS response now cached (backend hit ≤once/60s per slug); reverted the 4 pages from `force-dynamic` back to `revalidate` + `dynamicParams` ISR; `app/api/revalidate/route.ts` upgraded to also `revalidateTag("cms:{slug}"/"cms:all")` so the Master CMS cache-clear (single + bulk) and publish/save flow bust the **fetch data cache** (not just the route HTML) — published edits stay instant. Note: trek pages still render dynamically (pre-existing — `apiFetch`'s timeout-signal on trek profile/related/news forces it; page-level caching is served by Cloudflare `s-maxage=300`), but the fetchCMSPage Data Cache + tag-invalidation is the delivered win. VERIFIED prod-mode: trek/hi/about/contact/privacy/terms + `?_rsc=` all 200, 0 static-to-dynamic errors. Impact analysis on `fetchCMSPage` = CRITICAL (44 impacted / 18 public pages) — all verified safe. |
| **Step 81 comparison rebuild — pair table, no CMS pages (2026-07-14)** | **done** — Corrected #8 after user clarification ("agent creates clean URLs, NOT admin page_type=comparison pages"). Removed the CMS-page approach entirely; comparison URLs are now served **live** from trek data with pairs recorded in a lightweight table. Alembic `20260714_0054_trek_comparisons` (`trek_comparisons`: id/pair_slug unique+indexed/slug_a/slug_b/state/created_at/updated_at + 4 indexes); `TrekComparison` ORM (`modules/comparison/models.py`) + `db/base.py` registration. `modules/comparison/service.py` rewritten: `compute_comparison(page_a, page_b)` (live verdict + 7-row table from trek_guide backfill fields), `get_comparison_for_pair` (resolves registered pair → live payload, None if unregistered/unpublished), `upsert_pair` (idempotent row on canonical `{a}-vs-{b}`), `generate_comparisons_for_trek` (same-state top-3, excludes null-state + `%test%`), `backfill_all_comparisons`, `list_comparison_pairs` (aliased join → names/difficulties for both treks, only where both published). Routes: **public** `GET /api/v1/public/comparisons` (list) + `GET /api/v1/public/comparisons/{pair}` (live detail, 404 if unregistered); admin `POST /admin/comparisons/backfill` + `/generate/{slug}` now upsert pairs; publish trigger in `routes/cms.py` unchanged (dispatches `comparison.generate_for_trek`). Web: `lib/api.ts` `fetchComparisonPairs`/`fetchComparisonPair` + types; `compare/[pair]/page.tsx` renders from the live endpoint (`generateStaticParams` from pairs, SEO/AEO JSON-LD); `page.tsx` home section builds cards from pairs; `sitemap.ts` emits `/compare/{pair}` from `/public/comparisons` and drops the `comparison` PAGE_PREFIX mapping. **No `page_type=comparison` CMS pages created.** 7 rewritten `test_comparison.py` (TC-B-C01–C07, incl. live payload + public/admin routes), 763 backend pass (2 pre-existing refresh), `next build` ✓, live E2E ✓ (pair recorded → list + live detail return verdict + 7 rows). |
| **Step 81 gap-closure — editorial/compare/stub/app-tab (2026-07-14)** | **done** — 4 gaps closed after user cross-check of the 8-point pass. **PT2 (real editorial):** home "12,000 ft" spotlight now features a REAL published guide end-to-end — real name, seo_description excerpt, difficulty + duration chips, real Updated date, `Read the full guide` → `/trek/{slug}`; graceful fallback to the beginner-hub block when no CMS guide exists (`app/(public)/page.tsx`). **PT3 (personalized compare):** new `components/home/HomeComparisonsSection.tsx` (client) — cards link to clean `/compare/{a-vs-b}` pages, ranked by trending (server signal) + the viewer's most-viewed difficulty/region from `getBehaviorProfile()` (`lib/behavior-tracker.ts`); server builds `comparisonCards` from published `comparison` CMS pages + `comparisonFallback` interactive `/compare?slugs=` pairs when none exist yet; old hardcoded 2-col CTA removed. **PT4 (stub removal, migrate-to-CMS + 308 redirects):** deleted `modules/treks/data.py` (12 hardcoded stub treks); `modules/treks/service.py` rewritten CMS-backed (`list_treks`/`get_trek_by_slug` query published trek_guide pages with non-null `trek_state`, newest first, `_cms_to_trek` mapping incl. real `image`); `schemas/treks.py` `TrekSummary.image` added; `routes/treks.py` `get_treks`/`get_trek` now take `db`; `lib/trekApi.ts` uses real API image; `/trek/[slug]/page.tsx` legacy-slug 308 redirect generalized to fire for any unresolved slug (matches CMS slug-prefix or slugified trek_name) and moved `permanentRedirect()` OUTSIDE try/catch (was swallowing NEXT_REDIRECT). Mobile already stub-free (never calls `GET /treks`). `test_treks.py` rewritten self-contained CMS-backed; `test_treks_seasonal.py` slug-route test de-hardcoded. **PT8 (app tab):** `TrekTabBar.tsx` reports-tab label "Conditions" → "Trail Conditions" (numberOfLines 2 + centered so it fits equal-width). **PT4b (stricter real-trek filter):** `list_treks` (`_real_trek_query`) + `trek_state_counts` now also exclude `slug ILIKE '%test%'` fixture rows (production-safe — no real trek slug contains "test"); `test_treks.py::test_list_treks_excludes_test_fixture_slugs` proves the logic. Note: the dev DB's 3705 real trek_guides have NULL trek_state (never backfilled), so locally the trek list/regions read empty; production is unaffected (real treks have states — confirmed) and `backfill_all_trek_meta` can populate dev states if a working local home is needed. Validation: 763 backend pass (2 pre-existing refresh), `next build` ✓ 200/200, mobile `tsc` ✓, live smoke ✓ (`/api/v1/treks` real treks + image, `/treks/kedarkantha` 404→FE redirect, test slugs excluded). |
| **Security hardening — admin login rate limit + docs lockdown (2026-06-29)** | **done** — Security audit found 3 vulnerabilities: **(1) No rate limit on admin login** (CRITICAL): added `@limiter.limit("5/minute")` to `POST /api/v1/admin/auth/login` (`routes/admin_auth.py`) + `request: Request` param required by slowapi — brute-force now blocked after 5 wrong attempts/minute per IP; **(2) FastAPI docs publicly exposed** (MEDIUM): `main.py` FastAPI constructor updated to `docs_url="/docs" if settings.app_debug else None, redoc_url="/redoc" if settings.app_debug else None` — `/docs` and `/redoc` hidden in production when `APP_DEBUG=false`; **(3) Env-only fixes** (HIGH): `APP_DEBUG=false`, `AUTH_COOKIE_SECURE=true`, strong `AUTH_JWT_SECRET` must be set in DigitalOcean env vars. Full test suite pass; `next build` N/A (backend-only). |
| **TC Confirmations (2026-06-29)** | **Step 78 web UI — TC-F01–TC-F08 confirmed by user** (Trail Conditions section renders, sign-in gate, Add Report form, happy-path submit, validation, photo upload, admin moderation queue, mobile 375px layout). **Step 70 TC-F01–TC-F03 confirmed** (MonetizationSlot renders on free trek, GatedContent gate on premium page, free trek shows all sections). **Schema fix confirmed**: `amenityFeature` → `additionalProperty` (PropertyValue) for Live Trail Conditions / Community Trail Reports / Trek Buddy Matching — now visible in schema validators. |
| **Conditions enhancement pass (2026-06-29)** | **done** — 5 enhancements/bugfixes post Step 80: **(1) Coordinate re-seed** (Issue 4): `reseed_coordinates_task` Celery beat task (`conditions.reseed_coordinates`, daily 86400s) added to `worker/tasks/conditions.py` + `celery_app.py` — safe daily re-seed from TREK_COORDS dict, skips non-null coords so manual entries are never overwritten; **(2) Conditions sections in TOC + repositioning** (Issues 1+3): `trek/[slug]/page.tsx` — 3 TOC entries added (`live-conditions`/`trail-conditions`/`trek-buddy`); sections moved from full-width bottom position INTO the article left column (after AuthorBlock, before mobile CTA) with `not-prose scroll-mt-44` — now visible in the natural reading flow; **(3) JSON-LD schema** (Issue 2): trek page `trekSchema` spread with `amenityFeature` array — 3 × `LocationFeatureSpecification` entries (Live Weather & Conditions / Community Trail Reports / Trek Buddy Matching); **(4) Sitemap lastmod** (Issue 5): new `GET /api/v1/public/sitemap-treks?state=` endpoint in `sitemap_data.py` using `GREATEST(cms_pages.updated_at, trek_conditions.last_updated_at)` LEFT JOIN; `lib/state-sitemap.ts` updated to call new endpoint so Google re-crawls trek pages when conditions data refreshes; **(5) TrekSage user attribution** (Issue 6): `AIInteractionLogResponse` extended with `user_email`/`user_name`/`is_anonymous`; `list_ai_interaction_logs` service rewritten with LEFT JOIN `TreksageChatSession` (session_id=session_key) → `User` (user_id); `admin_treks.py` route simplified to use dict response; `admin/treksage-logs/page.tsx` — "User" column added (name+email for known users, Anonymous for anon), KPI row adds Known Users + Anonymous counts; `lib/api.ts` `AIInteractionLogEntry` extended with 3 user fields. 748/750 backend tests pass (2 pre-existing), `next build` ✓ zero errors |
| **M20 bugfix — status + fallback image + radius text (2026-06-29)** | **done** — 3 bugs found post-deploy: **(1) status mismatch** (root cause of blank cards): `routes/mobile.py` filtered `status=="live"` but CMS only produces `status=="published"` — zero enrichment rows ever matched, all treks fell back to null hero_image_url. Fixed to `"published"`. Test fixture updated. **(2) Wrong fallback image** `NearbyTreksSection.tsx`: `/images/placeholder-trek.webp` (non-existent) → `/images/trek-forest.jpg` (universal fallback). **(3) Stale heading** "within 200 km" → "within 300 km". 5/5 tests pass; `next build` ✓ clean. |
| **M20 bugfix — radius 200→300 km (2026-06-29)** | **done** — TC-F-M20-01 failed on prod: Delhi NCR is ~224 km from Nag Tibba (nearest trek), just outside the 200 km default. Changed `radius_km` default from 200 → **300** in `GET /api/v1/mobile/nearby` (`routes/mobile.py`). All 5 M20 tests still pass (all used explicit `radius_km` values). No frontend or mobile changes needed (neither passes `radius_km` explicitly). |
| **Step M20 — Nearby Treks (GPS)** (2026-06-29) | **done** — Backend: `_haversine_km` + `get_nearby_treks(lat, lon, radius_km, limit)` added to `modules/treks/service.py` (imports `TREK_COORDS` from `conditions/service.py` — 41 treks, no PostGIS needed); `NearbyTrekOut`/`NearbyTreksOut` schemas in `schemas/mobile.py`; `GET /api/v1/mobile/nearby` endpoint (no auth, lat/lon/radius_km/limit query params, enriches from CMS `status="published"` pages); 5 backend tests pass (753/755 total). Mobile: `expo-location@56.0.18` installed; `lib/location.ts` (foreground permission + 30-min AsyncStorage cache); `hooks/useNearbyTreks.ts` (React Query, staleTime=30min, `locationGranted` state); `NearbyTreksStrip.tsx` (horizontal card strip + distance badge overlay + permission-denied banner + skeleton); wired in `(home)/index.tsx` (after trending) + `browse/index.tsx` (in header before All Treks); `app.config.ts` — `NSLocationWhenInUseUsageDescription` in iOS infoPlist + `expo-location` plugin; `mobileApi.ts` — `NearbyTrekOut`/`NearbyTreksOut` interfaces added, `is_premium` made optional on `CMSPage`. Web: `NearbyTreksSection.tsx` (client component: `navigator.geolocation` → `/api/v1/mobile/nearby` → horizontal scrollable trek strip with distance badge; self-hides when denied/loading/no nearby treks; shows subtle banner when denied); wired in `/explore` page between trek grid and PersonalisedFeed. `npx tsc --noEmit` ✓ zero errors; `next build` ✓ zero errors. |
| **Mobile GatedContentOverlay wiring (Step 70 mobile parity)** (2026-06-29) | **done** — `apps/mobile/lib/mobileApi.ts`: `is_premium?: boolean` added to `CMSPage` interface; `apps/mobile/app/(tabs)/(home)/trek/[slug].tsx`: imports `GatedContentOverlay` (from M13) + `usePremium` hook; `isPremiumGated = trek.is_premium === true && !isPremium` gate inserted in content ternary — when `true`, renders `GatedContentOverlay` with `featureName="the full trek guide"` + `onUpgrade` → `/(tabs)/account/premium`; reports tab always accessible (not gated). `npx tsc --noEmit` ✓ zero errors. |
| **Step M19 — Live Trek Conditions (mobile)** (2026-06-26) | **done** — `hooks/useConditions.ts` (fetch + `AsyncStorage` cache with 6h TTL, offline fallback, 404→null graceful); `components/trek/ConditionsWidget.tsx` (inline widget: WMO emoji, current temp, humidity/wind, 3-day forecast scroll, trail/permit badges, "Details →" CTA); `components/conditions/LiveConditionsScreen.tsx` (full-screen overlay: current weather, 3-day forecast cards, trail status card + description, permit card + notes, condition summary, pull-to-refresh, "Offline" banner, last-updated timestamp); `app/(tabs)/(home)/trek/[slug].tsx` wired: `conditionsDetailVisible` state, `<ConditionsWidget>` in guide tab, `<LiveConditionsScreen>` as `StyleSheet.absoluteFill` overlay. `npx tsc --noEmit` ✓ zero errors. Shared backend: STEP-80 |
| **Step M18 — Trek Buddy Matching (mobile surfaces)** (2026-06-25) | **done** — `hooks/useBuddies.ts` (`buddyApi` namespace, `useTrekBuddies` + `useBuddyRequests` hooks, full TypeScript interfaces); `components/buddy/BuddySignalSheet.tsx` (pageSheet modal: month picker, group-size chips, experience toggle, notes); `components/buddy/BuddyListCard.tsx` (privacy-safe card, inline connect with optional message); `components/buddy/BuddyRequestSheet.tsx` (pending count badge, accept/decline actions, Open Chat CTA); `components/buddy/BuddyChatScreen.tsx` (pageSheet Modal, FlatList messages, 10s poll, KeyboardAvoidingView, send button); `components/buddy/TrekkerProfileModal.tsx` (formSheet modal: avatar/name/bio/stats/planning context, privacy notice, connect CTA); `app/(tabs)/(home)/trek/[slug].tsx` wired: buddy count block + signal sheet + profile modal + chat screen in guide tab; `app/(tabs)/account/index.tsx` wired: "Trek Buddy Requests" nav row → BuddyRequestSheet + BuddyChatScreen. `npx tsc --noEmit` ✓ zero errors. Shared backend: STEP-79 |
| **Step M17 — Trip Reports + Photo Gallery (mobile)** (2026-06-24) | **done** — 5th "Trail" tab added to `TrekTabBar` (`TrekTab` type extended with `"reports"`); `expo-image-picker@~56.0.18` + `expo-image-manipulator@~56.0.19` installed; `apiUploadFile<T>` export added to `mobileApi.ts`; `hooks/useReports.ts` (`useReports` hook: fetch paginated reports, `submitReport`, `uploadPhoto` via multipart FormData, `deleteReport`, `reload`); `components/reports/ConditionSummaryBanner.tsx` (condition bars, report count, last-report date); `components/reports/TripReportCard.tsx` (condition badge, date, title, body, photo thumbnails → gallery); `components/reports/PhotoGallery.tsx` (Modal + FlatList `pagingEnabled`, expo-image, full-screen with close button); `components/reports/PhotoPicker.tsx` (expo-image-picker + expo-image-manipulator resize→1920px, 3-photo limit, remove button); `components/reports/AddReportSheet.tsx` (slide-up Modal: trek date, condition radio, title, body with char counter, PhotoPicker, submit); `app/(tabs)/(home)/trek/[slug].tsx` wired: imports all 4 report components + `useReports`, `addReportVisible` state, "Trail" tab renders `ConditionSummaryBanner` + `TripReportCard` list + load-more + `AddReportSheet`; auth-gated "Add a report" CTA (sign-in prompt for guests). `npx tsc --noEmit` ✓ zero errors. Shared backend: STEP-78 |
| **Step M15 — Mobile CDP Analytics** (2026-06-24) | **done** — Alembic `20260623_0047_analytics_mobile_columns` (`platform`/`app_version` added to `analytics_events` + `analytics_sessions`, server_default='web'); `AnalyticsEvent`/`AnalyticsSession` ORM updated; `EventIn` schema extended with `platform`/`app_version`; `BatchEventIn.events` max_length bumped 20→50; `log_event` service passes platform/app_version through; `test_cdp.py::test_batch_ingest_exceeds_limit` updated to use 51 events; 4 new tests TC-B-M15-01–TC-B-M15-04 in `tests/test_cdp_mobile_m15.py`. Mobile: `lib/identity.ts` (anon ID via `Crypto.randomUUID()`/SecureStore + `setUserId`/`getUserId`); `lib/analyticsQueue.ts` (SQLite offline queue using `openDatabaseSync("ty_analytics_queue.db")`); `lib/analytics.ts` (13 convenience helpers incl. `trackCheckin`; batch flush to `/api/v1/analytics/events/batch`); `providers/AnalyticsProvider.tsx` (AppState session mgmt, 15-min background threshold, cold_start flag, queue flush on foreground); `hooks/useAnalytics.ts`; `app/_layout.tsx` wrapped in `<AnalyticsProvider>`; `AuthProvider.tsx` calls `setUserId(user.id)` on sign-in/up/Google and `setUserId(null)` on sign-out. 719/721 backend tests pass (2 pre-existing); tsc ✓; next build ✓ |
| **Step M14 — Push Notifications** (2026-06-23) | **done** — `expo-notifications@~56.0.18`; Alembic `20260623_0046_push_log` (`mobile_push_log` table: id/device_id/title/body/data/category/sent_at/status/error + 3 indexes); `MobilePushLog` ORM + `db/base.py` registration; `push_provider.py` (FCMProvider + APNsProvider, test mode when creds absent); `notifications/service.py` (`send_push`, `send_batch_push`, `get_devices_for_users`, `get_push_logs`, `_log_push`); Celery tasks `notifications.send_permit_alerts` (daily 86400s) + `notifications.send_seasonal_alerts` (weekly 604800s) + `notifications.send_news_alerts`; `admin_push.py` (`POST /api/v1/admin/push/send`, `GET /api/v1/admin/push/logs`); `FIREBASE_SERVICE_ACCOUNT_JSON`/`APNS_*` env vars in `config.py` + `.env.example`; `celery_app.py` extended with `app.worker.tasks.notifications` + 2 beat schedule entries; 5 new tests TC-B-M14-01–TC-B-M14-05; 707 backend tests pass. Mobile: `notificationService.ts` (permission on 2nd open via `Constants.executionEnvironment`, native token via `getDevicePushTokenAsync`, registered to existing `POST /api/v1/mobile/device`, `saveToInbox`/`getInbox`/`markAllRead`/`getUnreadCount`, `shouldShowBanner`/`shouldShowList` handler for expo-notifications v56); `useNotifications.ts` hook (inbox state, permission status, foreground listener, deeplink tap handler); `NotificationRow.tsx` (category icon badge, unread dot, relative timestamp); `app/notifications.tsx` (inbox screen, empty state, mark-all-read); `_layout.tsx` wired: `incrementOpenCount` → push token on 2nd open + `addNotificationReceivedListener`; `AppDrawer.tsx`: "Notifications" added to EXPLORE section; `notifications` Stack.Screen registered as modal; tsc ✓ 0 errors; next build ✓ |
| **Step M13 — Premium Subscription** (2026-06-22) | **done** — `react-native-iap@15.3.2` + `react-native-nitro-modules@0.35.9` (NitroModules arch — `expo-in-app-purchases` deprecated in SDK 56); `iapService.ts` wrapping v15 `fetchProducts`/`requestPurchase`/`getReceiptDataIOS`/`getAvailablePurchases`; `usePremium.ts` hook (`purchaseUpdatedListener` → backend verify → `queryClient.invalidateQueries`; test mode when IAP unavailable on simulator); `PremiumFeatureList` (free vs premium comparison table); `SubscribeButton` (status-aware); `GatedContentOverlay` (BlurView + lock icon + CTA); `account/premium.tsx` fully replaced (hero + plan selector monthly/annual + subscribe + restore + web fallback + active-premium state); backend: `POST /api/v1/subscriptions/iap/verify` + `POST /api/v1/subscriptions/iap/restore` (`iap_verify_purchase`/`iap_restore_purchases` in service, test mode when `APPLE_IAP_SHARED_SECRET` unset); `APPLE_IAP_SHARED_SECRET` + `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` env vars documented; `subscriptionApi` namespace added to `mobileApi.ts`; **Note: IAP purchase sheet requires real device + App Store Connect product setup (M22)** — simulator uses test mode; tsc ✓ 0 errors; TC-B16–B21 (6 new tests); 698 backend tests pass |
| **Mobile UI Pass 2 — Hamburger menu + tab bar + plan wizard light mode** (2026-06-22) | **done** — (1) **Hamburger menu**: `stores/drawerStore.ts` (Zustand open/close); `components/layout/AppDrawer.tsx` (right-side slide drawer, Spring animation, guest state: Sign In + Create Account CTAs, auth state: user profile header + Saved/Comparisons/Enquiries/Downloads/Settings + Sign Out, common: Browse Operators + Trek Resources + About + Safety); wired in `app/_layout.tsx` as overlay above all routes. (2) **Tab bar reduced to 5 items** (was 6): `saved` + `account` tabs set `href: null` in `(tabs)/_layout.tsx`; `CustomTabBar.tsx` filters both out of route loop; replaced with a static hamburger `☰` button at end of bar (calls `drawerStore.open()`); profile + saved features accessible via drawer. (3) **Plan wizard light mode**: `WizardStepLayout.tsx` all hardcoded `rgba(255,255,255,*)` colors replaced with `colors.textSecondary`/`colors.textMuted`/`colors.border`/`colors.borderSubtle`; `LeadCaptureForm.tsx` `infoText` hardcoded white → `colors.textSecondary`; wizard now fully visible in light mode. tsc ✓ zero errors |
| **Mobile Bugfix Pass 2 — Cross-platform personalization sync + Explore UX** (2026-06-23) | **done** — (1) **Cross-platform behavior sync**: new `behavior_profile JSON` column on `users` table (Alembic `20260623_0045`); `GET/PUT /api/v1/account/behavior-profile` routes + `BehaviorProfilePayload`/`BehaviorProfileResponse` Pydantic schemas + `get_behavior_profile`/`update_behavior_profile` service functions (cap 50 views server-side); 4 new backend tests TC-B21–B24; 702 backend tests pass. Mobile: `apiPut<T>` + `BehaviorProfileData` types + `accountApi.getBehaviorProfile`/`putBehaviorProfile` added to `mobileApi.ts`; `behaviorProfile.ts` rewrites with `recordTrekView(isAuthenticated)` fire-and-forget PUT + `pullAndMergeBehaviorProfile()` (remote+local merge, local wins on slug tie); `useBehaviorProfile.ts` detects login transition via `useRef(prevUserId)` and triggers pull on login. Trek detail `[slug].tsx` passes `!!user` to `recordTrekView`. Web: `behavior-tracker.ts` extended with `syncBehaviorProfileToBackend()` + `pullAndMergeBehaviorProfileFromBackend()`; `TrekViewTracker.tsx` calls `syncBehaviorProfileToBackend()` when `user` is truthy; `auth-context.tsx` calls `pullAndMergeBehaviorProfileFromBackend()` after `login`/`signup`/`loginWithGoogle`. (2) **Go Premium drawer entry**: `AppDrawer.tsx` — "Go Premium ⭐" saffron-accented row added to INFO_MENU, navigates to `/(tabs)/account/premium`. (3) **Explore filter prominence**: `FilterChips.tsx` — replaced inconsistent inactive-glass/active-solid pattern with a consistently prominent saffron-bordered pill ("Filter Treks"), fills solid saffron with white text + count when filters active. tsc ✓ 0 errors; next build ✓ 0 errors |
| **Mobile Bugfix Pass — Google Auth + 8 code/UX fixes** (2026-06-22) | **done** — (1) **Google auth** backend: `POST /api/v1/auth/mobile/google` (`MobileGoogleIn` schema → Google userinfo verify → `login_or_register_google_user` → `issue_mobile_token` → `MobileAuthOut`); frontend: `authApi.signInWithGoogle` replaced cookie-extraction hack with direct call to new endpoint. (2) **Trek offline parity**: `contentHtml`/`contentJson` columns added to SQLite (`db/schema.ts`, `db/client.ts`); `useTrekDetail.ts` caches and restores them — packing/permits/costs tabs now work offline. (3) **Biometric auth**: `AuthProvider.tsx` boot() reads `BIOMETRIC_KEY` after `loadStoredToken`; calls `promptBiometric()`; `clearAuth()` on failure; exposes `requiresBiometric` + `clearBiometricGate()`. (4) **Language toggle**: `mobileApi.getCmsPage` now accepts `lang` param; `useTrekDetail` reads `app_language` from AsyncStorage and passes it. (5) **Duplicate operators**: deleted `(home)/operators.tsx`; `OperatorsCTACard` route fixed to `/(tabs)/browse/operators`. (6) **Compare Sign in button**: was empty `onPress: () => {}`; fixed to `router.push("/(auth)/sign-in")`. (7) **Tab bar distortion**: icons-only (removed label Text + getLabelText render); active dot indicator; TAB_HEIGHT reduced 64→56px (iOS), 56→50px (Android); `navigation: any` type comment tightened. (8) **Plan results post-auth**: `_layout.tsx` AuthGate redirects to `/(tabs)/plan/results` after sign-in if wizard `intent`/`months` answers exist. tsc ✓ 0 errors; 11 M03 backend tests pass |
| **Step M10 — User Account** (2026-06-19) | **done** — Full account tab stack: `account/_layout.tsx` (Stack), `account/index.tsx` (dashboard: ProfileHeader + stats strip + menu rows + sign-out), `account/saved.tsx` (bookmarks via `GET /api/v1/account/bookmarks`, swipe-to-delete), `account/downloads.tsx` (digital products via `GET /api/v1/account/downloads` + download URL, Linking.openURL), `account/enquiries.tsx` (`GET /api/v1/auth/me/leads`), `account/premium.tsx` (coming-soon CTA), `account/settings.tsx` (name edit `PATCH /api/v1/auth/me`, EN/हिंदी toggle, biometric toggle, notifications link, Trail Letter newsletter `POST /api/v1/newsletter/subscribe`, legal links), `account/notifications.tsx` (6 per-category toggles, AsyncStorage), `account/privacy.tsx` (DPDP export + `DELETE /api/v1/auth/me/data`); 5 new components; `useAccount.ts` hook; `mobileApi.ts` extended (`apiPatch` + bookmark/download/newsletter/authMe APIs); tsc ✓ zero errors |
| **Step M09 — Plan My Trek Wizard** (2026-06-18) | **done** — Full 6-step native wizard replacing single-scroll `plan-my-trek.tsx`: `plan/_layout.tsx` (Stack), `plan/index.tsx` (hero intro + 6-step preview), `plan/step-1.tsx`–`step-5.tsx` (intent/month/duration/fitness+experience/region), `plan/step-6.tsx` (lead capture — skippable, POST /api/v1/leads/operator-help), `plan/results.tsx` (ranked cards, auth-gate, retry); `planWizardStore.ts` (Zustand — 7 fields + reset); 9 components in `components/plan/` (`WizardProgress`, `WizardStepLayout`, `IntentSelector`, `MonthSelector`, `DurationSelector`, `FitnessSliders`, `RegionSelector`, `LeadCaptureForm`, `PlanResultCard`); `leadsApi` + `OperatorHelpLeadPayload` added to `mobileApi.ts`; `CategoryHubRow` route fixed; stale `plan-my-trek` refs removed; tsc ✓ zero errors |
| **TrekSage hotfix 11 — enforce founder→authors routing in tool schema** (2026-06-18, commit 6a8087f) | **done** — (1) `get_site_info` tool description completely rewritten with explicit ROUTING RULES block (founder/people questions → "authors", company/mission → "about", policies → "privacy"/"terms"/"affiliate", etc.); (2) `topic` param: added `enum` of all 9 valid values + CRITICAL sentence: "Use 'authors' for any question about the founder, team member, or who built TrekYatra"; (3) `_call_get_site_info`: when `canonical=="about"`, adds `founder_note` field in response explicitly naming Deepesh Kumar Gupta — model has no excuse to skip it; 16/16 TrekSage tests pass |
| **TrekSage hotfix 12 — authors topic hardcoded (static page, not CMS)** (2026-06-18, commit 44562d7) | **done** — Root cause confirmed: `/about/authors` is 100% static React (hardcodes Deepesh Kumar Gupta's profile); no CMS page with `page_type="author"` exists in DB; `list_pages(page_type="author")` always returned empty. Fix: (1) `_AUTHORS_INFO` constant added to `treksage_agent.py` with full founder bio, skills, and contact; (2) `_SITE_INFO_MAP["authors"]`: changed from `{"page_type": "author"}` → `{"hardcoded": True}`; (3) `_call_get_site_info`: hardcoded branch now handles both "authors" and "safety" topics (returns constant without DB query); 16/16 TrekSage tests pass |
| **MCP HTTPS endpoint fix — ProxyHeadersMiddleware** (2026-06-18, commit f9d0a68) | **done** — Root cause: FastAPI behind Cloudflare+DO proxy was generating HTTP redirect URLs for `/mcp` → `/mcp/` because it couldn't see the original HTTPS scheme. Fix: added `ProxyHeadersMiddleware(trusted_hosts="*")` from `uvicorn.middleware.proxy_headers` to `app/main.py` (must be added before CORS middleware). Also added Claude.ai, ChatGPT, chat.openai.com to `_CORS_ORIGINS` for MCP client access. MCP endpoint now returns 200 directly at `https://api.trekyatra.co.in/mcp/` without 307 redirect. |
| **OpenAPI spec for ChatGPT Custom GPT** (2026-06-18, commit 8f22959) | **done** — Created `services/api/app/openapi_mcp.yaml` (OpenAPI 3.1.0 spec with 7 endpoints: listTreks, compareTreks, getTrekProfile, askTrekQuestion, recommendTreks, treksageChat, getTreksageHistory; 3 schemas: TrekSummary, TrekProfile, TrekRecommendation; server: `https://api.trekyatra.co.in/api/v1`). Added `GET /openapi-mcp.json` route in `main.py` serving YAML as JSON with `Access-Control-Allow-Origin: *`. ChatGPT Custom GPT published at `https://chatgpt.com/g/g-6a3501669eb88191a43b39b3afa4cac7-treksage-india-s-trek-planner`; privacy policy: `https://www.trekyatra.co.in/privacy`. |
| **Rate limiting + DDoS protection** (2026-06-18, commit 6b14854) | **done** — (1) `slowapi>=0.1.9` added to `pyproject.toml`; `Limiter`, `SlowAPIMiddleware`, `_rate_limit_exceeded_handler` wired in `main.py`; (2) `@limiter.limit("20/minute")` on `POST /treksage/chat` and `POST /treks/{slug}/ask`; `@limiter.limit("15/minute")` on `POST /treks/compare`; rate limit keyed by real client IP via `get_remote_address` (reads X-Forwarded-For from Cloudflare); 429 Too Many Requests on violation; (3) Trimmed 4 YAML descriptions exceeding ChatGPT's 300-char limit (getTrekProfile 340→<300, askTrekQuestion 348→<300, recommendTreks 343→<300, treksageChat 387→<300). |
| **TrekSage hotfix 10 — hard-route founder questions to authors** (2026-06-18, commit f5c1627) | **done** — (1) System prompt: replaced soft topic descriptions with explicit SITE INFORMATION ROUTING table (9 entries, each mapping question patterns to an exact topic); added CRITICAL rule: "who is the founder" → `topic="authors"`, never `topic="about"`; (2) `_SITE_INFO_ALIASES`: `"who": "about"` changed to `"who": "authors"` — who-questions are about people, not company mission; all founder/who_built/who_made aliases kept pointing to authors; 16/16 TrekSage tests pass |
| **TrekSage hotfix 9 — privacy/terms slug fixes + safety hardcoded response** (2026-06-18, commit e96b0f0) | **done** — (1) `_SITE_INFO_MAP["privacy"]`: slug `"privacy-policy"` → `"privacy"` (frontend fetchCMSPage uses "privacy"; old slug returned "Page not published"); (2) `_SITE_INFO_MAP["terms"]`: slug `"terms-of-service"` → `"terms"` (same mismatch); (3) `_SITE_INFO_MAP["safety"]`: removed broken slug `"safety-disclaimer"` (that page is fully static — no CMS record); replaced with `{"hardcoded": True}` → `_call_get_site_info` returns `_SAFETY_DISCLAIMER` constant (5 key safety rules + URL); 16/16 pass |
| **TrekSage hotfix 8 — @tailwindcss/typography registered + site-info context 2000→4000** (2026-06-18, commit 512be0d) | **done** — (1) `tailwind.config.ts`: `require("@tailwindcss/typography")` added to plugins array — package was installed but never registered; `prose prose-lg` class generated zero CSS so `<h2>/<h3>/<ul>/<table>` on all CMS informational pages (/about, /contact, /privacy, /terms, /methodology, /affiliate-disclosure) rendered as unstyled plain text; (2) Typography theme extension maps prose colors to CSS variables (--foreground, --accent, --border); (3) `_call_get_site_info` strip_html truncation 2000→4000 chars for longer pages; next build ✓ |
| **TrekSage hotfix 7 — altitude blank + HTML sections + founder + methodology slug** (2026-06-18, commit 9514a38) | **done** — (1) `TrekRecommendation.max_altitude_ft: int` added to schemas/plan.py; `_to_rec` wires `page.trek_max_altitude_ft` so canvas card altitude stat shows instead of "—"; (2) `_extract_content_sections`: removed `re.sub` HTML strip — keeps raw HTML so CMS tables/lists/bold render in TrekDetailPanel; (3) `_extract_faqs`: reverted HTML stripping — raw HTML answers match section behaviour; (4) `TrekDetailPanel SectionAccordion`: replaced `<p whitespace-pre-line>` with `dangerouslySetInnerHTML` + Tailwind arbitrary selectors for table/list/heading/link styles; (5) `_SITE_INFO_MAP["methodology"]` slug corrected `"editorial-methodology"` → `"methodology"` (slug was renamed in CMS; old slug returned "not published"); (6) `_SITE_INFO_ALIASES` adds founder/founders/who_founded/who_built/who_made → "authors" so "who is the founder" queries resolve to the Authors page (Deepesh Kumar Gupta profile); (7) system prompt: authors topic description updated + WHAT NOT TO DO guardrail against "[the content appears cut off]" hallucinations; TC-B32 assertion inverted to expect HTML preserved; 49/49 pass; next build ✓ |
| **TrekSage hotfix 6 — recommend_treks broken + FAQ HTML + images + plan query** (2026-06-18, commit 0b4f136) | **done** — (1) CRITICAL: `_call_tool("recommend_treks")` iterated `PlanRecommendResponse` directly and accessed `r.trek` (non-existent attr) — every plan query silently errored → "couldn't find results" fallback; fixed to iterate `response.recommendations` and pass `r` to `_slim_profile`; (2) `_slim_profile` now uses `getattr` for all fields — safe on both `TrekProfile` and `TrekRecommendation`; (3) System prompt: TOOL SELECTION section + GET_SITE_INFO SPECIFICITY section; (4) post-processor: "couldn't find" fallback only fires if tools returned no trek results; (5) `_extract_faqs` strips HTML from answer (same `re.sub` as `_extract_content_sections`); (6) All trek card hero images switched from `<Image fill>` to `<img>` in TreksageChat + TrekDetailPanel (bypasses Next.js image optimization, definitively fixes 4th-occurrence blank image issue); TC-B48–B49 added; 16/16 TrekSage tests pass; next build ✓ |
| **TrekSage hotfix 5 — double nav + blank images + get_site_info** (2026-06-18, commit 75d029c) | **done** — (1) Route group move: `app/(public)/treksage/` → `app/(treksage)/treksage/` — removes SiteLayout inheritance, eliminates double Header + Footer/TrekSageWidget on /treksage; (2) Image placeholders: gradient background + Mountain icon opacity /15→/40 on both CanvasTrekCard and ChatTrekCard; (3) `get_site_info` tool added to treksage_agent.py — fetches About/Contact/Privacy/Terms/Affiliate/Safety/Methodology/GearReviews/AuthorNetwork from CMS via `_SITE_INFO_MAP` + `_SITE_INFO_ALIASES`; guardrails in system prompt (cite URL, no invented contacts, legal disclaimer); `page.is_published` → `page.status != "published"` bug fixed; TC-B45–B47 (3 new tests); 690/692 pass; next build ✓ |
...
- `FIREBASE_SERVICE_ACCOUNT_JSON`, `APNS_KEY_ID`, `APNS_TEAM_ID` — env vars documented in M14 (test mode active until set on DO)
...
| App Platform — remaining env vars: Stripe, Razorpay, Google OAuth (mobile/Firebase), SMS/Twilio (ANTHROPIC_API_KEY ✅ done; SMTP ✅ done via Step 68; Google OAuth web ✅ done) | pending |
...
| DO Console action: run alembic upgrade head on production (adds search_events table) | ✅ done on prod (2026-06-29) |
...
| 70 | **Step 70 — Component Wiring: MonetizationSlot (Z02) + GatedContent (Z03)** — (1) Replaced hardcoded `AffiliateRail` + static 3-item `gearItems` with `<Suspense><MonetizationSlot slug={slug} sourcePage={pageUrl} /></Suspense>` — now calls `fetchIntent` → picks AffiliateRail/LeadForm/NewsletterCapture dynamically; (2) Added `isPremiumGated = cmsPage?.is_premium === true` gate wrapping entire article body (why-this-trek → trek-buddy) — premium pages show `GatedContent` blur overlay + Upgrade CTA, free pages render normally; mobile Plan CTA always visible. `next build` ✅ zero errors. No backend changes. | **done** (2026-06-29) |
...
- ~~**Celery worker must be restarted** after deploy to register `trek_intelligence.backfill_all_trek_meta` task.~~ ✅ **DONE on prod (2026-06-29)** — user confirmed worker restarted.
- ~~After restart, admin must click **"Backfill All Treks"** on `/admin/trek-data` once — this populates the 51 trek guides with AI-drafted structured fields.~~ ✅ **DONE on prod (2026-06-29)** — user confirmed backfill action was run.
...
- ~~GatedContent component not yet wired~~ → **Done in Step 70 (2026-06-29)** — wired as `isPremiumGated` gate in trek/[slug]/page.tsx
...
What remains:
- ~~Wire MonetizationSlot into trek detail page CTA slot~~ → **Done in Step 70 (2026-06-29)** — replaces static AffiliateRail with dynamic MonetizationSlot via Suspense
