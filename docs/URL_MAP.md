...
| `/seasons/[slug]` | Seasonal hub | `seasonal_hub` (optional overlay) | **Rich, UNIQUE code-first page (2026-08-04 rebuild).** Per-season content in `lib/season-content.ts` (`SEASON_CONTENT`: distinct hero, tagline, intro, why-trek, best-regions, month-by-month table, packing, weather per slug — spring/summer/monsoon/autumn/winter + december/may). Renders H1 + multiple H2/H3, a month table, bullets, a live season-matched trek grid (`/treks/seasonal?season=`), dynamic stat strip, and **generated season-specific FAQs** → `FAQPage` + `ItemList` + `BreadcrumbList` schema. No LLM dependency; a `seasonal_hub` CMS page only adds an optional in-depth block (rendered when >400 chars). Fixed the old bugs: monsoon title said "in Maharashtra", shared hero images, identical body/cards. |
| `/trek-types/[slug]` | Cluster hub (Trek Category) | `cluster_hub` | **EXCLUSIVELY the 6 curated thematic categories** (`category_meta` / `lib/categories`: beginner-friendly-treks, weekend-treks, high-altitude-treks, lake-treks, snow-treks, family-treks). **Any other slug 404s** (2026-08-04 fix) — per-trek/keyword-cluster trek-types URLs duplicate `/trek/{slug}` detail pages and were removed. Renders a live member-trek grid (`/treks/by-cluster?category=`) + FAQ/ItemList schema. Optional CMS overlay via the hub generator. **Keyword_cluster generation removed.** |
