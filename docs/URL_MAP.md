...
| `/app` | iOS app landing page | — | **NEW (2026-08-20, owner-confirmed §17)** — download/AEO page for the live iOS app: hero + smart App Store download button, features, screenshots, AEO FAQ. Emits `MobileApplication` + `FAQPage` + `BreadcrumbList` schema. In sitemap. Paired with `/llms.txt`, the `apple-itunes-app` Smart App Banner meta, and App Store URL in Organization `sameAs`. |
...
| `/regions/[slug]` | Regional hub | `regional_hub` (optional) | **STRICT: exclusively the curated regions (`lib/regions.REGIONS`); any other slug 404s** (2026-08-04) — no synthesised junk region pages; `groupStateCounts` is curated-only so home/nav/sitemap only link to real hubs. Added a unique "Why trek in {region}" H2 section (`whyTrek`). **Dynamic, code-rendered hub (2026-08-03 rewrite).** Single source of truth = `lib/regions.ts` (`REGIONS` + `resolveRegion`/`regionSlugForState`/`groupStateCounts`). Canonical slugs: uttarakhand, himachal, kashmir, ladakh, maharashtra, sikkim, karnataka (India) + `nepal`, `pakistan`, `tibet` (International Himalaya). A trek's `trek_state` (incl. composite international values like "Koshi Province, Nepal / Tibet, China") resolves to the right hub by exact/`matchWord` substring match — **no per-region code**. Stats (trek count, beginner routes, peak season, permits), FAQs, and schema (`TouristDestination` + `FAQPage` + `BreadcrumbList`) are all generated live from published trek data. An optional `regional_hub` CMS page at slug `regions/{slug}` enriches with editor content + custom FAQs (not required). **Canonical dedupe:** the region page emits `<link rel=canonical>` to the short hub slug; the old auto-slugified composite URLs (`/regions/gilgit-baltistan-pakistan`, `/regions/koshi-province-nepal-tibet-china`, etc.) **301-redirect** to their canonical hub (`next.config.mjs`). Indexed via the root `sitemap.xml` (one `/regions/{slug}` per region with published treks, derived from `trek-state-counts`). Region hero images are real per-region photos in `/public/images/region-*.webp`. |
| `/seasons/[slug]` | Seasonal hub | `seasonal_hub` (optional overlay) | **Rich, UNIQUE code-first page (2026-08-04).** EXCLUSIVELY the **5 canonical seasons** (spring/summer/monsoon/autumn/winter) — `lib/season-content.ts`. **Any other slug 404s**; the old `december`/`may` month hubs were **removed** (duplicated winter/summer) and **301-redirect** to winter/summer. Renders distinct hero + H1, dynamic stat strip, why-trek, best-regions cards, live season-matched trek grid, month-by-month table, packing bullets + weather, and **generated FAQs** → `FAQPage` + `ItemList` + `Breadcrumb` schema. Fixed: monsoon title "in Maharashtra", shared images, identical body. |
| `/trek-types/[slug]` | Cluster hub (Trek Category) | `cluster_hub` | **EXCLUSIVELY the 6 curated thematic categories** (`lib/categories` + `lib/category-content.ts`: beginner-friendly-treks, weekend-treks, high-altitude-treks, lake-treks, snow-treks, family-treks). **Any other slug 404s.** **Rich, UNIQUE page (2026-08-04):** distinct hero + H1, stat strip, why-choose, planning tips (bullets), best-regions cards, live member-trek grid (`/treks/by-cluster?category=`), **generated FAQs** → `FAQPage` + `ItemList` + `Breadcrumb`. Per-trek/keyword-cluster trek-types URLs were removed (duplicated `/trek/{slug}`); keyword_cluster generation removed. **In nav:** "Trek Categories" mega-menu column + footer links. Light public theme (matches seasons/regions). |
...
| `/api/v1/admin/hubs/clusters/catalog` | **Admin GET** — the 6 curated Trek Categories (`category_meta.CATEGORIES`) with `has_page` (powers "Generate Missing Trek Category Hubs"). **Keyword_clusters removed** (2026-08-04) |
| `/api/v1/admin/hubs/clusters/generate` | **Admin POST** — generate a `cluster_hub` from a **curated `{category_slug}` only** via ClusterContentAgent. `cluster_id` rejected (422). Existing curated hubs regenerate via `/{slug}/regenerate` (2026-08-04) |
| `/api/v1/treks/by-cluster?category=` | **Public GET** — curated Trek Category slug matches treks by predicate (`category_meta`). Powers `/trek-types/[slug]` (2026-08-04) |
