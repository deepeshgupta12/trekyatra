...
- `apps/web-next/next.config.mjs` -> `productionBrowserSourceMaps: true` — **TEMPORARY (revert after hydration debugging)**. Emits `*.js.map` in prod so minified React errors demap to source. Runtime JS/HTML unchanged (cannot break live); trade-off is larger build + publicly readable source. blast radius: build-only.
