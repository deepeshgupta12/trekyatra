...
- **`react-native-reanimated@4.3.1` + `react-native-worklets@0.8.3`** — Expo SDK 56 selected versions via `npx expo install`. Reanimated 4.x requires worklets as a peer dep (podspec validates its presence). v4.x rewrites native layer without folly/coro dependency so RN 0.85.3 compiles cleanly. NativeWind v4's `react-native-css-interop` requires reanimated at runtime so it cannot be deferred to M07. Babel plugin `react-native-reanimated/plugin` added to `babel.config.js` in M01.
- **`react` and `react-dom` pinned to exact `19.2.3`** — `react-native@0.85.3` bundles `react-native-renderer@19.2.3`; React requires an exact version match. Using `^19.0.0` resolved to `19.2.7` causing a runtime crash. Always pin `react` to the exact version matching the bundled `react-native-renderer`.
