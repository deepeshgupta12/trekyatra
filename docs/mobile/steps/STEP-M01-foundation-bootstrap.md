...
- **`react-native-reanimated` upgraded to 4.3.1** (Expo SDK 56 selected via `npx expo install`). v3.16.x was incompatible with RN 0.85.3 (folly/coro header removed); v4.x rewrites the native layer without folly/coro dependency. NativeWind v4's `react-native-css-interop` requires reanimated at runtime (`makeMutable`, `withRepeat`, `withSequence`), so it cannot be deferred. Babel plugin `react-native-reanimated/plugin` added to `babel.config.js` in M01 (not M07 as originally planned) because NativeWind needs it.
