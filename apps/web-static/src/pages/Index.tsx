...
import { Search, MapPin, Calendar, Mountain, Sparkles, ArrowRight, Star, Shield, FileCheck, Backpack, Wallet, Compass, ChevronRight, Clock, TrendingUp } from "lucide-react";
import { Button } from "@/components/ui/button";
import { SiteLayout } from "@/components/layout/SiteLayout";
import { TrekCard } from "@/components/trek/TrekCard";
import { treks, featuredImage } from "@/data/treks";
...
const regions = [
  { name: "Himachal Pradesh", count: "48 treks", image: himachal, slug: "himachal" },
  { name: "Uttarakhand", count: "62 treks", image: uttarakhand, slug: "uttarakhand" },
  { name: "Kashmir & Ladakh", count: "29 treks", image: kashmir, slug: "kashmir" },
  { name: "Sahyadris", count: "70+ treks", image: sahyadri, slug: "maharashtra" },
  { name: "Sikkim & NE", count: "24 treks", image: ladakh, slug: "sikkim" },
];

const Home = () => {
  const trending = treks.slice(0, 4);
  const beginner = treks.filter(t => t.beginner);
  const monsoon = treks.filter(t => t.state === "Maharashtra");
    ...
    <SiteLayout>
      ...
      <section className="relative min-h-[88vh] flex items-end overflow-hidden">
        <div className="absolute inset-0">
          <img src={featuredImage} alt="Himalayan dawn ridge with two trekkers" width={1920} height={1280} className="w-full h-full object-cover" />
      ...
      <Section
        ...
        title="Treks Indians are obsessing over right now"
        cta={{ label: "View all treks", to: "/explore" }}
      >
        ...
        <div className="container-wide">
          <div className="grid lg:grid-cols-3 gap-8 items-center mb-12">
            <div className="lg:col-span-2">
              ...
              <h2 className="font-display text-4xl md:text-5xl font-semibold leading-tight">
                Five trust pillars. Every trek you choose.
              </h2>
            </div>
            <p className="text-muted-foreground text-base">
              We don't just tell you where to go. We tell you when, how, what to pack, what it'll cost, and what permits you need — all updated weekly.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            {[
              ...
              <Link key={p.to} to={p.to} className="group p-6 bg-card rounded-2xl border border-border lift">
      ...
      {/* REGIONS */}
      <Section
        eyebrow="Explore by geography"
        title="India's great trekking regions"
        cta={{ label: "All regions", to: "/regions/himachal" }}
      >
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {regions.map((r) => (
            <Link key={r.slug} to={`/regions/${r.slug}`} className="group relative h-72 overflow-hidden rounded-2xl lift">
      ...
      <Section
        ...
        title="Beginner-friendly treks across India"
        cta={{ label: "Beginner guide", to: "/beginner" }}
        muted
      >
        ...
        <div className="container-wide">
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div className="relative h-[520px] rounded-2xl overflow-hidden stack-shadow">
              <img src={summit} alt="Trekker at Himalayan summit" loading="lazy" width={1200} height={1200} className="w-full h-full object-cover" />
              <div className="absolute top-5 left-5 px-3 py-1 rounded-full bg-accent text-accent-foreground text-xs uppercase tracking-widest font-semibold">
                Editorial spotlight
              </div>
            </div>
            <div>
              ...
              <h2 className="font-display text-4xl md:text-5xl font-semibold leading-tight mb-6">
                What nobody tells you about your first trek above 12,000 ft.
              </h2>
              <p className="text-muted-foreground text-lg leading-relaxed mb-6">
                Acclimatisation isn't optional. Cotton kills above the snowline. And the cheapest operator is rarely the safest. Our editor walks you through the 11 things that decide whether your first Himalayan trek becomes a story you tell forever — or one you don't tell at all.
              </p>
      ...
      {/* MONSOON */}
      <Section
        eyebrow="Sahyadri season"
        title="Monsoon treks in Maharashtra & Karnataka"
        cta={{ label: "Monsoon collection", to: "/seasons/monsoon" }}
      >
      ...
      <section className="py-16 md:py-24 bg-gradient-pine text-surface relative overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <svg width="100%" height="100%" viewBox="0 0 1200 600" preserveAspectRatio="none">
            ...
            <path d="M0,500 L120,460 L240,480 L360,420 L480,460 L600,380 L720,430 L840,370 L960,440 L1080,380 L1200,430 L1200,600 L0,600 Z" fill="hsl(var(--accent))" opacity="0.5" />
          </svg>
        </div>
        <div className="container-wide relative grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h2 className="font-display text-4xl md:text-5xl font-semibold leading-tight mb-6">
              Kedarkantha or Brahmatal? Hampta or Bhrigu?
            </h2>
            <p className="text-surface/80 text-lg leading-relaxed mb-8 max-w-xl">
              Our side-by-side comparisons score difficulty, scenery, snow probability, beginner-fit, cost, and logistics. So you pick once — and pick right.
            </p>
            <div className="flex gap-3">
              <Link to="/compare"><Button variant="hero" size="lg">Browse comparisons</Button></Link>
              <Link to="/explore"><Button variant="glass" size="lg">Explore all treks</Button></Link>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {[["Kedarkantha","Brahmatal"],["Hampta Pass","Bhrigu Lake"],["Valley of Flowers","Hampta Pass"],["Kashmir Lakes","Sandakphu"]].map(([a,b]) => (
              <Link key={a+b} to="/compare" className="glass-dark rounded-2xl p-5 hover:bg-surface/10 transition-colors">
        ...
        <div className="container-narrow">
          <div className="relative rounded-3xl overflow-hidden p-10 md:p-16 bg-gradient-twilight text-surface text-center">
            ...
            <div className="relative">
              ...
              <p className="text-surface/80 max-w-xl mx-auto mb-8 text-lg">
                Tell us your fitness, dates, and budget. We'll match you to the right trek and the right operator — free, in 48 hours.
              </p>
              <Link to="/plan"><Button variant="hero" size="xl"><Sparkles className="h-4 w-4" /> Plan My Trek</Button></Link>
            </div>
          </div>
        </div>
      </section>
...
const Section = ({ eyebrow, title, cta, children, muted = false }: { eyebrow: string; title: string; cta?: { label: string; to: string }; children: React.ReactNode; muted?: boolean }) => (
  <section className={`py-16 md:py-24 ${muted ? "bg-surface-muted" : ""}`}>
    <div className="container-wide">
      <div className="flex items-end justify-between mb-10 gap-6">
        ...
        {cta && (
          <Link to={cta.to} className="hidden md:flex items-center gap-1 text-sm font-medium text-foreground/80 hover:text-accent transition-colors whitespace-nowrap">
