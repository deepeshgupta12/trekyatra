        ...
        <div className="glass border-b border-border/60">
          <div className="container-wide flex h-16 items-center gap-6">
              ...
              {primaryLinks.map((link) => (
                ...
                >
                  <NavLink
                    to={link.to}
                    className={({ isActive }) =>
                      `px-3 py-2 text-sm font-medium rounded-full transition-all flex items-center gap-1 ${
                        isActive ? "text-accent" : "text-foreground/80 hover:text-foreground hover:bg-muted"
                      }`
                    }
                  >
                    {link.label}
            ...
            <div className="flex items-center gap-1">
              <Link to="/saved" className="hidden md:flex h-10 w-10 items-center justify-center rounded-full hover:bg-muted text-foreground/70 hover:text-foreground transition-colors" aria-label="Saved">
                <Bookmark className="h-4 w-4" />
              </Link>
              <Link to="/auth/sign-in" className="hidden md:flex h-10 w-10 items-center justify-center rounded-full hover:bg-muted text-foreground/70 hover:text-foreground transition-colors" aria-label="Account">
            ...
            >
                  ...
                  <div key={section.title}>
                    ...
                    <ul className="space-y-2.5">
                      {section.items.map((item) => (
                        <li key={item.to}>
                          <Link to={item.to} className="text-sm text-foreground/80 hover:text-accent transition-colors">
                            {item.label}
                          </Link>
                        </li>
                      ))}
              ...
              <div className="bg-muted/40 border-t border-border">
                <div className="container-wide py-4 flex items-center justify-between text-xs text-muted-foreground">
                  ...
                  <Link to="/explore" className="text-accent font-semibold hover:underline">Browse all treks →</Link>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="absolute right-0 top-0 bottom-0 w-[90%] max-w-sm bg-background shadow-elevated overflow-y-auto animate-fade-up">
            ...
            <nav className="px-5 pb-5 space-y-1">
              ...
              <div className="pt-3 mt-3 border-t border-border space-y-1">
                <Link to="/saved" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-muted">
                  <Bookmark className="h-4 w-4" /> Saved Treks
                </Link>
                <Link to="/auth/sign-in" onClick={() => setMobileOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-muted">
