...
function SignUpForm() {
  ...
  async function handleGoogleToken(accessToken: string) {
    ...
    try {
      ...
      // Google signup skips onboarding — go directly to ?next or account
      router.push(next);
      router.refresh();
    } catch (err: unknown) {
  ...
  return (
    <>
      ...
      <p className="text-sm text-muted-foreground mt-3 text-center">Already have an account? <Link href={signInHref} className="text-accent font-medium">Sign in</Link></p>
    </>
  );
}
