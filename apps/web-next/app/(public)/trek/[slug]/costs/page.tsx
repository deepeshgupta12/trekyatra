...
async function findCostPage(trekSlug: string): Promise<CMSPage | null> {
  const candidates = [
    `${trekSlug}-cost-guide`,
    `${trekSlug}-costs`,
    `${trekSlug}-budget`,
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (page.status === "published" && page.page_type === "cost_guide") return page;
    } catch { /* try next */ }
  }
  return null;
}
      ...
      <section className="bg-gradient-to-br from-background via-background to-accent/5 border-b border-border py-14">
        <div className="container-wide max-w-3xl">
          ...
          <div className="flex items-center gap-3 mt-5 mb-4">
            ...
            <span className="text-xs uppercase tracking-widest text-muted-foreground font-medium">Cost guide</span>
          </div>
      ...
      <section className="py-12">
        <div className="container-wide max-w-3xl">
          <article className="prose prose-lg max-w-none">
            {sec.cost_estimate && <div className="not-prose cms-section mb-8" dangerouslySetInnerHTML={{ __html: sec.cost_estimate }} />}
            {page.content_html && !sec.cost_estimate && (
          ...
          <div className="mt-10 flex items-center gap-4 text-sm text-muted-foreground">
            ...
            <span>·</span>
            <Link href="/costs" className="hover:text-foreground">All cost guides</Link>
          </div>
