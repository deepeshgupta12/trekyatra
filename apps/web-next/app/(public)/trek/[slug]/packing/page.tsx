...
export default async function TrekPackingPage({ params }: { params: { slug: string } }) {
  const page = await findPackingPage(params.slug);
  // No dedicated packing guide for this trek → the packing info lives inline on the main trek page.
  // 308 there instead of 404 (kills the historical GSC 404s for /trek/{slug}/packing).
