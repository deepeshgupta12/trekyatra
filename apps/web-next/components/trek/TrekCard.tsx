      ...
      <article className={`relative overflow-hidden rounded-2xl bg-card border border-border lift ${featured ? "h-[480px]" : "h-[420px]"}`}>
        ...
        <div className="absolute top-4 left-4 right-4 flex justify-between">
          ...
          <button
            ...
            disabled={loading}
            aria-label={bookmarked ? "Remove bookmark" : "Save trek"}
            className={`h-9 w-9 rounded-full glass-dark flex items-center justify-center transition-colors ${
              bookmarked ? "bg-accent text-white" : "text-surface hover:bg-accent"
