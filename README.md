...
| Trek Category (cluster) hub generation in `/admin/hubs` — `ClusterContentAgent` (hybrid); curated taxonomy (`category_meta`: beginner/weekend/high-altitude/lake/snow/family, predicate-matched) **+** keyword_clusters; "Generate Missing Trek Category Hubs" panel; `/treks/by-cluster?category=`; `/trek-types/[slug]` live trek grid + ItemList | Done |
