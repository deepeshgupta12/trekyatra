...
| Trek Category (cluster) hubs `/trek-types/[slug]` — **rich + unique** (`lib/category-content.ts`): distinct hero, stat strip, why-choose, planning tips, best-regions, live trek grid, generated FAQs → `FAQPage`+`ItemList`+`Breadcrumb`. **Curated 6 categories only** (404 otherwise; keyword_cluster gen removed). Generated in `/admin/hubs` | Done |
| Hub slugs strict — `/regions`, `/seasons`, `/trek-types` **404 for any non-curated slug** (no synthesised/junk URLs); `/seasons/{december,may}` removed + 301 → winter/summer; `groupStateCounts` curated-only | Done |
| Seasonal hubs `/seasons/[slug]` — rich, **unique per season** (code-first, `lib/season-content.ts`): distinct hero, why-trek, best-regions, month-by-month table, packing + weather, dynamic stat strip, live season-matched trek grid, generated FAQs → `FAQPage`+`ItemList`+`Breadcrumb` schema | Done |
| Hub SEO parity — `/trek-types/[slug]` (curated categories) emits `FAQPage`+`ItemList` JSON-LD | Done |
| Consistent hub↔trek mapping — one canonical source (`hubs/season_meta.py` + `hubs/cluster_meta.py`): season = backfill months (`trek_best_months`→`trek_open_months`→`trek_season`), 5 seasons; cluster = `cluster_id` FK + `trek_themes` fallback. Seasonal + cluster hubs + home tabs now render REAL CMS+backfill treks (`/treks/seasonal?season=`, `/treks/by-cluster`) with trek names in `ItemList` schema | Done |
...
| **Critical-path trims** — display/mono fonts not preloaded (3→1 font preloads, frees LCP bandwidth); removed unused Spaces preconnect; reverted the no-op mount-gate | Done |
...
| CDP admin — funnels | `POST /api/v1/admin/cdp/funnels/dynamic`, `GET /funnels/templates`, `GET/POST /funnels/saved`, `POST /funnels/saved/{id}/run`, `DELETE /funnels/saved/{id}` | Admin auth |
| CDP admin — cohorts | `GET /api/v1/admin/cdp/cohorts` (opt `?source=`/`?behavior_event=`), `POST /cohorts/custom` | Admin auth |
| CDP admin — attribution | `GET /api/v1/admin/cdp/attribution?days=` (first/last/linear channel credit) | Admin auth |
...
| CDP (Step 64) | `analytics_events` (+`ip_hash` salted, `os`, `country` now populated for web), `analytics_sessions`, `user_traits` (+`lifecycle_stage`, `engagement_score`, `lead_score`, `traits_computed_at` — P1 depth), `attribution_touchpoints`, `gsc_performance`, `saved_funnels` (P2 — named funnel defs) |
...
| GatedContentOverlay wired to mobile trek detail — `is_premium` gate via `usePremium`, reports tab always accessible | Done (M20 pass) |
| Nearby Treks (GPS) — `lib/location.ts` (foreground permission + 30-min cache), `useNearbyTreks` hook, `NearbyTreksStrip` (home + browse); haversine backend `GET /api/v1/mobile/nearby`; web `NearbyTreksSection` on /explore | Done (M20) |
...
| **SEO — GSC 404 deep-path fix (final)** | Diagnosed by status-checking all 108 GSC-reported URLs against production: 76 already fixed (66→308, 29→410), 11 genuinely live 404s. `middleware.ts` `checkDeepPath()` — durable `/trek/{slug ending -YYYY-MM}` → 301 `/news/{slug}` (replaces a 4-slug curated list; 280 news articles exist), 410 for unknown top-level segments, for sub-paths of childless routes (`ROUTE_CHILDREN` mirrors `app/(public)`), and for invalid `/trek/{slug}/{sub}`; `GONE_PATHS` for bare-word dead trek slugs. `/hi/{trek,packing,guides}/[slug]` → 308 English (zero Hindi pages published). **Killed the active generator:** trek-page `SiteNavigationElement` JSON-LD advertised `/packing`,`/permits`,`/costs` for every trek (189 dead URLs — zero such CMS pages exist) + 2 sidebar links on 280 news pages — both removed. Interlink audit: 176/185 live internal links → 200. Backend: link sanitizer un-gated from `trek_guide` to all page types; `published_url` uses new `public_path_for()` instead of hardcoded `/trek/`; +6 tests. Separate commit: `/packing|permits|guides/[slug]` soft-404s (200 for any slug) → `notFound()` | Done — 2026-09-22 |
| **M20 — Nearby Treks (GPS) + Mobile GatedContentOverlay** | Backend: `_haversine_km` + `get_nearby_treks` in `treks/service.py` (imports TREK_COORDS from conditions); `NearbyTrekOut`/`NearbyTreksOut` in `schemas/mobile.py`; `GET /api/v1/mobile/nearby` (no auth, lat/lon/radius_km/limit); 5 tests (753/755 pass). Mobile: `expo-location`; `lib/location.ts`; `hooks/useNearbyTreks.ts`; `NearbyTreksStrip.tsx`; wired home + browse; iOS NSLocationWhenInUseUsageDescription + expo-location plugin. Web: `NearbyTreksSection.tsx` on /explore. Mobile gate: `is_premium` gate via `usePremium` + `GatedContentOverlay` in trek detail; reports tab always accessible. `tsc --noEmit` ✅; `next build` ✅ | Done — 2026-06-29 |
