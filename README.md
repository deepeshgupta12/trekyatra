...
| Trek Category (cluster) hub generation in `/admin/hubs` — `ClusterContentAgent`; **curated taxonomy ONLY** (`category_meta`: beginner/weekend/high-altitude/lake/snow/family, predicate-matched). `/trek-types/[slug]` **404s for non-curated slugs** (no per-trek duplicate URLs); live trek grid + ItemList; keyword_cluster generation removed | Done |
